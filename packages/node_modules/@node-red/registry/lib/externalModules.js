...
function registerSubmodel(type, submodelConfig){
    submodelTypes[type] = submodelConfig
}
...
function requireModule(module) {
    ...
    if(module.includes("/")){ 
        var slash_index = module_subpath.indexOf("/")
        module = module_subpath.slice(0, slash_index)
    }
...
async function checkFlowDependencies(flowConfig) {
    ...
    const checkedSubmodels = {};
    while (nodes.length > 0) {
        let n = nodes.shift();
        if (subflowTypes[n.type] && !checkedSubflows[n.type]) {
            checkedSubflows[n.type] = true;
            nodes = nodes.concat(subflowTypes[n.type].flow)
        } else if(submodelTypes[n.type] && !checkedSubmodels[n.type]){
            checkedSubmodels[n.type] = true;
            nodes = nodes.concat(submodelTypes[n.type].flow)
...
module.exports = {
    ...
    registerSubmodel: registerSubmodel,

...
function init(_settings) {
    ...
    if (settings.externalModules && settings.externalModules.modules) {
        if (settings.externalModules.modules.allowList || settings.externalModules.modules.denyList) {
            ...
            console.log("Allowedlist:   " ,installAllowList)
            installDenyList = settings.externalModules.modules.denyList;
            console.log("Denied list:   ", installDenyList)
        }
    ...
    // geminos
    console.log("GEM registry loaded allow list: " ,installAllowList)
    console.log("GEM registry loaded denied list: " ,installDenyList)
...
function requireModule(module) {
    console.log("GEM In require module: ", module)
    var module_subpath = module
    console.log("GEM module subpath:  ", module_subpath)
    if(module === "typedb-client/TypeDB")
    {
        console.log("FORGOT TO CHECK MODULE NAME PARSE")
        module_subpath = "typedb-client/TypeDB"
        module = "typedb-client"
    }
    ...
    if (BUILTIN_MODULES.indexOf(module) !== -1) {
        ...
        console.log("GEM second if ")

        return require(module);
    }
    if (!knownExternalModules[module]) {
        console.log("GEM uitl require modul: 3rd if")
        console.log(knownExternalModules[module])
        console.log(knownExternalModules)
        const e = new Error("Module not allowed");
        e.code = "module_not_allowed";
        throw e;
    }
    const externalModuleDir = path.resolve(path.join(settings.userDir || process.env.NODE_RED_HOME || ".", EXTERNAL_MODULES_DIR));
    const moduleDir = path.join(externalModuleDir,"node_modules",module_subpath);
    console.log("GEM module directory   ", moduleDir)
    return require(moduleDir);
}

function parseModuleName(module) {
    console.log("GEM PARSING MODULE NAME")
    var match = /((?:@[^/]+\/)?[^/@]+)(?:@([\s\S]+))?/.exec(module);
    console.log("GEM matched to: ", match)
    if (match) {

...
var submodels = require("./submodel");
...
function registerSubmodel(nodeSet, submodel) {
    var nodeSetInfo = getFullNodeInfo(nodeSet);

    const result = submodels.register(nodeSet,submodel);

    if (submodelModules.hasOwnProperty(result.type)) {
        throw new Error(result.type+" already registered");
    }
    ...
    submodelModules[result.type] = result;
    externalModules.registerSubmodel(result.type,submodel);
...
function getNodeConstructor(type) {
    ...
    if (!config || (config.enabled && !config.err)) {
        return nodeConstructors[type] || subflowModules[type] || submodelModules[type];
    }
    return null;
}

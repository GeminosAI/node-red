...
RED.editor = (function() {
    ...
    function validateNode(node) {
        ...
        if (node.type.indexOf("subflow:")===0) {
        ...
        }else if(node.type.indexOf("submodel:")===0){
            submodel = RED.nodes.submodel(node.type.substring(9));
            isValid = submodel.valid;
            hasChanged = submodel.changed;
            if (isValid === undefined) {
                isValid = validateNode(submodel);
                hasChanged = submodel.changed;
            }
            validationErrors = validateNodeProperties(node, node._def.defaults, node);
            node.valid = isValid && validationErrors.length === 0;
            ...
            var submodelNodes = RED.nodes.filterNodes({z:node.id});
            for (var i=0;i<submodelNodes.length;i++) {
                isValid = submodelNodes[i].valid;
                hasChanged = submodelNodes[i].changed;
                if (isValid === undefined) {
                    isValid = validateNode(submodelNodes[i]);
                    hasChanged = submodelNodes[i].changed;
                }
                node.valid = node.valid && isValid;
                node.changed = node.changed || hasChanged;
            }
            var submodelInstances = RED.nodes.filterNodes({type:"submodel:"+node.id});
            var modifiedTabs = {};
            for (i=0;i<submodelInstances.length;i++) {
                submodelInstances[i].valid = node.valid;
                submodelInstances[i].changed = submodelInstances[i].changed || node.changed;
                submodelInstances[i].dirty = true;
                modifiedTabs[submodelInstances[i].z] = true;
            }
            Object.keys(modifiedTabs).forEach(function(id) {
                var submodel = RED.nodes.submodel(id);
                if (submodel) {
                    validateNode(submodel);
                }
            });
        }
        if (oldValue !== node.valid || oldChanged !== node.changed) {
            ...
            if(submodel){
                submodel = RED.nodes.submodel(node.z);
                if (submodel) {
                    validateNode(submodel);
                }
            }
    ...
    function prepareEditDialog(node,definition,prefix,done) {
        ...
        if (definition.credentials || /^subflow:/.test(definition.type) || /^submodel:/.test(definition.type)) {
            if (node.credentials) {
                ...
                getNodeCredentials(node.type, node.id, function(data) {
                    if (data) {
                        ...
                        if (!/^submodel:/.test(definition.type)) {
                            populateCredentialsInputs(node, definition.credentials, node.credentials, prefix);
                        }
                    }
                    completePrepare();
    ...
    function getEditStackTitle() {
        ...
        for (var i=editStack.length-1;i<editStack.length;i++) {
            ...
            if (node.type === 'group') {
                ...
                label = RED._("submodel.editSubmodel",{name:RED.utils.sanitize(node.name)})
                ...
                var submodel = RED.nodes.submodel(node.type.substring(9));
                label = RED._("submodel.editSubmodelInstance",{name:RED.utils.sanitize(submodel.name)} )
    ...
    function buildEditForm(container,formId,type,ns,node) {
        ...
        if (type === "submodel-template") {
            ...
            // TODO: this needs to happen later in the dialog open sequence
            //       so that credentials can be loaded prior to building the form
            RED.submodel.buildEditForm(type,node);
        }
    ...
    function buildAppearanceForm(container,node) {
        ...
        if (node.type === "submodel") {
            ...
            $("<label/>", {
                for: "submodel-appearance-input-category",
                "data-i18n": "editor:submodel.category"
            }).appendTo(categoryRow);
            var categorySelector = $("<select/>", {
                id: "submodel-appearance-input-category"
            }).css({
                width: "250px"
            }).appendTo(categoryRow);
            $("<input/>", {
                type: "text",
                id: "submodel-appearance-input-custom-category"
            }).css({
                display: "none",
                "margin-left": "10px",
                width: "calc(100% - 250px)"
            ...
            $("#submodel-appearance-input-category").on("change", function() {
                var val = $(this).val();
                if (val === "_custom_") {
                    $("#submodel-appearance-input-category").width(120);
                    $("#submodel-appearance-input-custom-category").show();
                } else {
                    $("#submodel-appearance-input-category").width(250);
                    $("#submodel-appearance-input-custom-category").hide();
                }
            })

            $("#submodel-appearance-input-category").val(node.category||"submodels");
            var userCount = 0;
            var submodelType = "submodel:"+node.id;
            ...
            //     }
            // });
            $("#red-ui-editor-submodel-user-count")
                .text(RED._("submodel.submodelInstances", {count:node.instances.length})).show();
        }
...
var buildingEditDialog = false;
    ...
    function showEditDialog(node, defaultTab) {
        ...
        var trayOptions = {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    click: function() {
                        ...
                        if (type === "subflow" || type === "submodel") {
                            var old_env = editing_node.env;
                            var new_env;
                            if(type ==="subflow"){
                                new_env = RED.subflow.exportSubflowInstanceEnv(editing_node);
                            }else if(type ==="submodel"){
                                new_env = RED.submodel.exportSubmodelInstanceEnv(editing_node);
                            }
                        ...
                        if (changed) {
                            ...
                            if(type ==="subflow"){
                                ...
                                var submodelInstances = null;
                                if(activeSubmodel){
                                    submodelInstances = [];
                                    RED.nodes.eachNode(function(n){
                                        if (n.type == "submodel:"+RED.workspaces.active()){
            ...
            open: function(tray, done) {
                ...
                if (/^submodel:/.test(node.type)) {
                    var submodelPropertiesTab = {
                        id: "editor-submodel-envProperties",
                        label: RED._("editor-tab.envProperties"),
                        name: RED._("editor-tab.envProperties"),
                        content: $('<div>', {id:"editor-submodel-envProperties-content",class:"red-ui-tray-content"}).appendTo(editorContent).hide(),
                        iconClass: "fa fa-list"
                    };
                    editorTabs.addTab(submodelPropertiesTab);
                    // This tab is populated by the oneditprepare function of this
                    // subflow. That ensures it is done *after* any credentials
                    // have been loaded for the instance.
                }
        ...
        if (type === 'submodel') {
            var id = editing_node.type.substring(9);
            trayOptions.buttons.unshift({
                class: 'leftButton',
                text: RED._("submodel.edit"),
                click: function() {
                    RED.workspaces.show(id);
                    skipInfoRefreshOnClose = true;
                    $("#node-dialog-ok").trigger("click");
    ...
    function showEditConfigNodeDialog(name,type,id,prefix) {
        ...
        var activeSubmodel = RED.nodes.submodel(RED.workspaces.active());
        if(activeSubmodel){
            configNodeScope = activeSubmodel.id;
        }
        ...
        var trayOptions = {
            ...
            open: function(tray, done) {
                ...
                prepareEditDialog(editing_config_node,node_def,"node-config-input", function() {
                    ...
                    var activeSubmodel = RED.nodes.submodel(RED.workspaces.active());
                    if (activeSubmodel) {
                        tabSelect.append('<option disabled data-i18n="sidebar.config.submodels"></option>');
                        RED.nodes.eachSubmodel(function(ws) {
                            var workspaceLabel = ws.name;
                            if (nodeUserFlows[ws.id]) {
                                workspaceLabel = "* "+workspaceLabel;
                            }
    ...
    function updateConfigNodeSelect(name,type,value,prefix) {
        ...
        if (prefix) {
            ...
            if (button.length) {
                ...
                if (!activeWorkspace) {
                    activeWorkspace = RED.nodes.subflow(RED.workspaces.active()) ? RED.nodes.subflow(RED.workspaces.active()) : RED.nodes.submodel(RED.workspaces.active());
                }
    ...
    function showEditSubmodelDialog(submodel) {
        if (buildingEditDialog) { return }
        buildingEditDialog = true;
        var editing_node = submodel;
        editStack.push(submodel);
        RED.view.state(RED.state.EDITING);
        var submodelEditor;
        var finishedBuilding = false;
        var trayOptions = {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    click: function() {
                        ...
                        var newName = $("#submodel-input-name").val();
                        ...
                        var newDescription = submodelEditor.getValue();
                        ...
                        var newCategory = $("#submodel-appearance-input-category").val().trim();
                        if (newCategory === "_custom_") {
                            newCategory = $("#submodel-appearance-input-custom-category").val().trim();
                            if (newCategory === "") {
                                newCategory = editing_node.category;
                            }
                        }
                        if (newCategory === 'submodels') {
                            newCategory = '';
                        }
                        if (newCategory != editing_node.category) {
                            changes['category'] = editing_node.category;
                        ...
                        if (oldColor !== newColor) {
                            ...
                            changed = true;
                            RED.utils.clearNodeColorCache();
                            if (editing_node.type === "submodel") {
                                var nodeDefinition = RED.nodes.getType(
                                    "submodel:" + editing_node.id
                                );
                                nodeDefinition["color"] = newColor;
                            }
                        }
                        ...
                        var old_env = editing_node.env;
                        var new_env = RED.submodel.exportSubmodelTemplateEnv($("#node-input-env-container").editableList("items"));
                        ...
                        var newMeta = RED.submodel.exportSubmodelModuleProperties(editing_node);
                        ...
                        if (changed) {
                            ...
                            RED.nodes.eachNode(function(n) {
                                if (n.type == "submodel:"+editing_node.id) {
                                    submodelInstances.push({
                                        id:n.id,
                                        changed:n.changed
                                    })
                            ...
                            RED.events.emit("submodels:change",editing_node);
                            RED.nodes.dirty(true);
                            var historyEvent = {
                                t:'edit',
                                node:editing_node,
            ...
            resize: function(size) {
                ...
                if (envContainer.length) {
                    // var form = $(".red-ui-tray-content form").height(size.height - 50 - 40);
                    var rows = $("#dialog-form>div:not(#submodel-env-tabs-content)");
                    var height = size.height;
                    for (var i=0; i<rows.size(); i++) {
                        height -= $(rows[i]).outerHeight(true);
                    }
            ...
            open: function(tray, done) {
                ...
                if (editing_node.type === "submodel") {
                    ...
                    $("<span/>").text(" ").appendTo(span);
                    $("<i/>", {
                        id: "red-ui-editor-submodel-user-count"
                    }).appendTo(span);
                }
                ...
                editorTabs.addTab(moduleTab);
                RED.submodel.buildModuleForm(moduleTab.content, editing_node);
                ...
                var descriptionTab = {
                    ...
                    iconClass: "fa fa-file-text-o",
                    onchange: function() {
                        submodelEditor.focus();
                    }
                };
                editorTabs.addTab(descriptionTab);
                submodelEditor = buildDescriptionForm(descriptionTab.content,editing_node);
                ...
                buildEditForm(nodePropertiesTab.content,"dialog-form","submodel-template", undefined, editing_node);
                trayBody.i18n();
                getNodeCredentials("submodel", submodel.id, function(data) {
                    if (data) {
                        submodel.credentials = data;
                        submodel.credentials._ = $.extend(true,{},data);
                    }
                    $("#submodel-input-name").val(submodel.name);
                    RED.text.bidi.prepareInput($("#submodel-input-name"));
    ...
    return {
        ...
        editSubmodel: showEditSubmodelDialog, 

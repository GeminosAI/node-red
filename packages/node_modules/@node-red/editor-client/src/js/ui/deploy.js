    ...
    function () {
        ...
        function init(options) {
            ...
            if (type == "default") {
                ...
                const mainMenuItems = [
                    { id: "deploymenu-item-full", toggle: "deploy-type", icon: "red/images/deploy-full.svg", label: RED._("deploy.full"), sublabel: RED._("deploy.fullDesc"), selected: false, onselect: function (s) { if (s) { changeDeploymentType("full") } } },
                    { id: "deploymenu-item-flow", toggle: "deploy-type", icon: "red/images/deploy-flows.svg", label: RED._("deploy.modifiedFlows"), sublabel: RED._("deploy.modifiedFlowsDesc"), selected: true, onselect: function (s) { if (s) { changeDeploymentType("flows") } } },
                    { id: "deploymenu-item-node", toggle: "deploy-type", icon: "red/images/deploy-nodes.svg", label: RED._("deploy.modifiedNodes"), sublabel: RED._("deploy.modifiedNodesDesc"), selected: false, onselect: function (s) { if (s) { changeDeploymentType("nodes") } } },
                    null
                ]
            ...
            $('#red-ui-header-button-deploy').on("click", function (event) {
                ...
                try {
                    RED.typeql.deploy(deploymentType)
                    RED.causeway.loadNodes()
                    save()
                } catch (exception) {
                    console.log("catche exception at RED.deploy after calleing RED.typeql.deploy", exception)
                }
            });
        ...
        function getNodeInfo(node) {
            ...
            if (node.z) {
                ...
                if (!tab) {
                    tab = RED.nodes.subflow(node.z) ? RED.nodes.subflow(node.z) : RED.nodes.submodel(node.z)
                    tabLabel = tab.name;
                } else {
                    tabLabel = tab.label;
                }
        ...
        function save(skipValidation, force) {
            ...
            $.ajax({
                ...
                // geminos
                RED.nodes.eachSubmodel(function (submodel) {
                    submodel.changed = false;
                })
                // end of geminos

 ...
 **/
 RED.palette.editor = (function () {
    ...
    function SemVer(ver) {
      ...
      this.parts = [
        new SemVerPart(groups[1]),
        new SemVerPart(groups[3]),
        new SemVerPart(groups[5]),
        new SemVerPart(groups[7]),
        new SemVerPart(groups[9]),
      ];
    }
    ...
    function formatUpdatedAt(dateString) {
      ...
      if (months === 0) {
        ...
        return RED._(
          "palette.editor.times.year" + (years > 1 ? "s" : "") + "MonthsV",
          { y: years, count: months }
        );
      }
    }
    ...
    function _refreshNodeModule(module) {
      if (!nodeEntries.hasOwnProperty(module)) {
        ...
        if (nodeEntry) {
          ...
          for (var setName in moduleInfo.sets) {
            if (moduleInfo.sets.hasOwnProperty(setName)) {
                ...
                if (set.enabled) {
                  ...
                  if (def && def.color) {
                    ...
                    swatch.css({
                      border:
                        "1px solid " +
                        getContrastingBorder(swatch.css("backgroundColor")),
                    });
                  }
                }
              }
              ...
              setElements.setRow.toggleClass(
          ...
          var nodeCount =
            activeTypeCount === typeCount
              ? typeCount
              : activeTypeCount + " / " + typeCount;
          nodeEntry.setCount.text(
            RED._("palette.editor.nodeCount", {
              count: typeCount,
              label: nodeCount,
            })
          );
        ...
        if (moduleInfo.pending_version) {
          nodeEntry.versionSpan
            .html(
              moduleInfo.version +
                ' <i class="fa fa-long-arrow-right"></i> ' +
                moduleInfo.pending_version
            )
            .appendTo(nodeEntry.metaRow);
          nodeEntry.updateButton
            .text(RED._("palette.editor.updated"))
            .addClass("disabled")
          ...
          if (
            ...
            nodeEntry.updateButton.text(
              RED._("palette.editor.update", {
    ...
    function handleCatalogResponse(err, catalog, index, v) {
      ...
      if (catalogueCount > 1) {
        $(".red-ui-palette-module-shade-status").html(
          RED._("palette.editor.loading") +
            "<br>" +
            catalogueLoadStatus.length +
            "/" +
            catalogueCount
        );
      }
      if (catalogueLoadStatus.length === catalogueCount) {
        if (catalogueLoadErrors) {
          RED.notify(
            RED._("palette.editor.errors.catalogLoadFailed", { url: catalog }),
            "error",
            false,
            8000
          );
    ...
    function initInstallTab() {
      if (loadedList.length === 0) {
        ...
        $(".red-ui-palette-module-shade-status").text(
          RED._("palette.editor.loading")
        );
        var catalogues = RED.settings.theme("palette.catalogues") || [
        ...
        if (catalogues.length > 1) {
          $(".red-ui-palette-module-shade-status").html(
            RED._("palette.editor.loading") + "<br>0/" + catalogues.length
          );
        }
    ...
    function sortModulesRelevance(A, B) {
      ...
      var i =
        A.info.index.indexOf(currentFilter) - B.info.index.indexOf(currentFilter);
      if (i === 0) {
        return sortModulesAZ(A, B);
      }
      return i;
    ...
    function init() {
      ...
      var settingsAllowList = RED.settings.get(
        "externalModules.palette.allowList"
      );
      ...
      RED.events.on("registry:node-type-added", function (nodeType) {
        if (!/^subflow:/.test(nodeType) && !/^submodel:/.test(nodeType)) {
      ...
      RED.events.on("nodes:add", function (n) {
        if ( (!/^subflow:/.test(n.type )) && (!/^submodel:/.test(n.type))) {
    ...
    function createNodeTab(content) {
      var modulesTab = $("<div>", {
        class: "red-ui-palette-editor-tab",
      }).appendTo(content);
      ...
      var filterDiv = $("<div>", { class: "red-ui-palette-search" }).appendTo(
        modulesTab
      );
      filterInput = $(
      ...
      nodeList = $("<ol>", {
        ...
        .editableList({
          ...
          addItem: function (container, i, object) {
            ...
            if (entry) {
              var headerRow = $("<div>", {
                class: "red-ui-palette-module-header",
              }).appendTo(container);
              var titleRow = $(
                '<div class="red-ui-palette-module-meta red-ui-palette-module-name"><i class="fa fa-cube"></i></div>'
              ).appendTo(headerRow);
              $("<span>").text(entry.name).appendTo(titleRow);
              var metaRow = $(
                '<div class="red-ui-palette-module-meta red-ui-palette-module-version"><i class="fa fa-tag"></i></div>'
              ).appendTo(headerRow);
              var versionSpan = $("<span>").text(entry.version).appendTo(metaRow);
  
              var errorRow = $(
                '<div class="red-ui-palette-module-meta red-ui-palette-module-errors"><i class="fa fa-warning"></i></div>'
              )
                .hide()
                .appendTo(headerRow);
              var errorList = $(
                '<ul class="red-ui-palette-module-error-list"></ul>'
              ).appendTo(errorRow);
              var buttonRow = $("<div>", {
                class: "red-ui-palette-module-meta",
              }).appendTo(headerRow);
              var setButton = $(
                '<a href="#" class="red-ui-button red-ui-button-small red-ui-palette-module-set-button"><i class="fa fa-angle-right red-ui-palette-module-node-chevron"></i> </a>'
              ).appendTo(buttonRow);
              var setCount = $("<span>").appendTo(setButton);
              var buttonGroup = $("<div>", {
                class: "red-ui-palette-module-button-group",
              }).appendTo(buttonRow);
  
              var updateButton = $(
                '<a href="#" class="red-ui-button red-ui-button-small"></a>'
              )
                .text(RED._("palette.editor.update"))
                .appendTo(buttonGroup);
              updateButton.attr(
                "id",
                "up_" + Math.floor(Math.random() * 1000000000)
              );
              updateButton.on("click", function (evt) {
                ...
                update(
                  entry,
                  loadedIndex[entry.name].version,
                  loadedIndex[entry.name].pkg_url,
                  container,
                  function (err) {}
                );
              });
              ...
              var removeButton = $(
                ...
                .text(RED._("palette.editor.remove"))
                .appendTo(buttonGroup);
              removeButton.attr(
                "id",
                "up_" + Math.floor(Math.random() * 1000000000)
              ...
              var enableButton = $(
                ...
                .text(RED._("palette.editor.disableall"))
                .appendTo(buttonGroup);
  
              var contentRow = $("<div>", {
                class: "red-ui-palette-module-content",
              }).appendTo(container);
              var shade = $(
                '<div class="red-ui-palette-module-shade hide"><img src="red/images/spin.svg" class="red-ui-palette-spinner"/></div>'
              ).appendTo(container);
              ...
              setList.forEach(function (setName) {
                ...
                var setRow = $("<div>", {
                  class: "red-ui-palette-module-set",
                }).appendTo(contentRow);
                var buttonGroup = $("<div>", {
                  class: "red-ui-palette-module-set-button-group",
                }).appendTo(setRow);
                var typeSwatches = {};
                set.types.forEach(function (t) {
                  var typeDiv = $("<div>", {
                    class: "red-ui-palette-module-type",
                  }).appendTo(setRow);
                  typeSwatches[t] = $("<span>", {
                    class: "red-ui-palette-module-type-swatch",
                  }).appendTo(typeDiv);
                  $("<span>", { class: "red-ui-palette-module-type-node" })
                    .text(t)
                    .appendTo(typeDiv);
                });
                var enableButton = $(
                    ...
                    changeNodeState(set.id, newState, shade, function (xhr) {
                      if (xhr) {
                        if (xhr.responseJSON) {
                          RED.notify(
                            RED._(
                              "palette.editor.errors." +
                                (newState ? "enable" : "disable") +
                                "Failed",
                              { module: id, message: xhr.responseJSON.message }
                            )
                          );
                        }
                      }
                  ...
                  changeNodeState(
                    ...
                    function (xhr) {
                      if (xhr) {
                        if (xhr.responseJSON) {
                          RED.notify(
                            RED._("palette.editor.errors.installFailed", {
                              module: id,
    ...
    function createInstallTab(content) {
      var installTab = $("<div>", {
        class: "red-ui-palette-editor-tab hide",
      }).appendTo(content);
      ...
      var toolBar = $("<div>", {
        class: "red-ui-palette-editor-toolbar",
      }).appendTo(installTab);
  
      var searchDiv = $("<div>", { class: "red-ui-palette-search" }).appendTo(
        installTab
      );
      searchInput = $(
        '<input type="text" data-i18n="[placeholder]palette.search"></input>'
      )
        .appendTo(searchDiv)
        .searchBox({
          delay: 300,
          change: function () {
            ...
            if (searchTerm.length > 0) {
              ...
              searchInput.searchBox(
                "count",
                filteredList.length + " / " + loadedList.length
              );
            } else {
              searchInput.searchBox("count", loadedList.length);
              packageList.editableList("empty");
      ...
      $("<span>")
        .text(RED._("palette.editor.sort") + " ")
        .appendTo(toolBar);
      var sortGroup = $('<span class="button-group"></span>').appendTo(toolBar);
      var sortRelevance = $(
        '<a href="#" class="red-ui-palette-editor-install-sort-option red-ui-sidebar-header-button-toggle selected"><i class="fa fa-sort-amount-desc"></i></a>'
      ).appendTo(sortGroup);
      var sortAZ = $(
        '<a href="#" class="red-ui-palette-editor-install-sort-option red-ui-sidebar-header-button-toggle" data-i18n="palette.editor.sortAZ"></a>'
      ).appendTo(sortGroup);
      var sortRecent = $(
        '<a href="#" class="red-ui-palette-editor-install-sort-option red-ui-sidebar-header-button-toggle" data-i18n="palette.editor.sortRecent"></a>'
      ).appendTo(sortGroup);
      ...
      var refreshButton = $(
        '<a href="#" class="red-ui-sidebar-header-button"><i class="fa fa-refresh"></i></a>'
      ).appendTo(refreshSpan);
      ...
      packageList = $("<ol>", {
        style: "position: absolute;top: 79px;bottom: 0;left: 0;right: 0px;",
      })
        .appendTo(installTab)
        .editableList({
          addButton: false,
          ...
          addItem: function (container, i, object) {
            if (object.count) {
              $("<div>", { class: "red-ui-search-empty" })
                .text(
                  RED._("palette.editor.moduleCount", { count: object.count })
                )
                .appendTo(container);
              return;
            }
            if (object.more) {
              ...
              var moreRow = $("<div>", {
                class: "red-ui-palette-module-header palette-module",
              }).appendTo(container);
              var moreLink = $('<a href="#"></a>')
                .text(RED._("palette.editor.more", { count: object.more }))
                .appendTo(moreRow);
              moreLink.on("click", function (e) {
                ...
                for (
                  var i = object.start;
                  i < Math.min(object.start + 10, object.start + object.more);
                  i++
                ) {
                  packageList.editableList("addItem", filteredList[i]);
                }
            ...
            if (object.info) {
              ...
              $("<span>")
                ...
                .appendTo(titleRow);
              $(
                '<a target="_blank" class="red-ui-palette-module-link"><i class="fa fa-external-link"></i></a>'
              )
                .attr("href", entry.url)
                .appendTo(titleRow);
              var descRow = $(
                '<div class="red-ui-palette-module-meta"></div>'
              ).appendTo(headerRow);
              $("<div>", { class: "red-ui-palette-module-description" })
              ...
              $(
                '<span class="red-ui-palette-module-version"><i class="fa fa-tag"></i> ' +
                  entry.version +
                  "</span>"
              ).appendTo(metaRow);
              $(
                '<span class="red-ui-palette-module-updated"><i class="fa fa-calendar"></i> ' +
              ...
              if (entry.types && entry.types.length > 0) {
                for (var i = 0; i < entry.types.length; i++) {
                  var nodeset = RED.nodes.registry.getNodeSetForType(
                    entry.types[i]
                  );
                  if (nodeset) {
              ...
              var installButton = $(
                ...
                .text(RED._("palette.editor.install"))
                .appendTo(buttonGroup);
              ...
              if (nodeEntries.hasOwnProperty(entry.id)) {
                ...
                RED.popover.create({
                  ...
                  content: RED._("palette.editor.conflictTip", {
      ...
      if (
        RED.settings.get("externalModules.palette.allowUpload", true) !== false
      ) {
        var uploadSpan = $('<span class="button-group">').prependTo(toolBar);
        var uploadButton = $(
          '<button type="button" class="red-ui-sidebar-header-button red-ui-palette-editor-upload-button"><label><i class="fa fa-upload"></i><form id="red-ui-palette-editor-upload-form" enctype="multipart/form-data"><input name="tarball" type="file" accept=".tgz"></label></button>'
        ).appendTo(uploadSpan);
        ...
        var uploadToolbar = $(
          '<div class="red-ui-palette-editor-upload"></div>'
        ).appendTo(installTab);
        var uploadForm = $("<div>").appendTo(uploadToolbar);
        var uploadFilename = $(
          '<div class="placeholder-input"><i class="fa fa-upload"></i> </div>'
        ).appendTo(uploadForm);
        var uploadFilenameLabel = $("<span></span>").appendTo(uploadFilename);
        var uploadButtons = $(
          '<div class="red-ui-palette-editor-upload-buttons"></div>'
        ).appendTo(uploadForm);
        $('<button class="editor-button"></button>')
          .text(RED._("common.label.cancel"))
          .appendTo(uploadButtons)
          .on("click", function (evt) {
            evt.preventDefault();
            uploadToolbar.slideUp(200);
            uploadInput.val("");
          });
        $('<button class="editor-button primary"></button>')
          .text(RED._("common.label.upload"))
          .appendTo(uploadButtons)
          .on("click", function (evt) {
            evt.preventDefault();
            ...
            var buttonRow = $(
              '<div style="position: relative;bottom: calc(50% + 17px); padding-right: 10px;text-align: right;"></div>'
            ).appendTo(spinner);
            $('<button class="red-ui-button"></button>')
            ...
            RED.eventLog.startEvent(
              RED._("palette.editor.confirm.button.install") +
                " : " +
                uploadInput[0].files[0].name
            );
      ...
      $(
        '<div id="red-ui-palette-module-install-shade" class="red-ui-palette-module-shade hide"><div class="red-ui-palette-module-shade-status"></div><img src="red/images/spin.svg" class="red-ui-palette-spinner"/></div>'
      ).appendTo(installTab);
    }
    ...
    function update(entry, version, url, container, done) {
      ...
      var notification = RED.notify(
        RED._("palette.editor.confirm.update.body", { module: entry.name }),
        {
          modal: true,
          fixed: true,
          buttons: [
            ...
            },
            {
              ...
              class:
                "primary red-ui-palette-module-install-confirm-button-update",
              click: function () {
                ...
                installNodeModule(entry.name, version, url, function (xhr) {
                  ...
                  if (xhr) {
                    if (xhr.responseJSON) {
                      var notification = RED.notify(
                        RED._("palette.editor.errors.updateFailed", {
                          module: entry.name,
                          message: xhr.responseJSON.message,
                        }),
                        {
    ...
    function remove(entry, container, done) {
      ...
      var notification = RED.notify(
        RED._("palette.editor.confirm.remove.body", { module: entry.name }),
        {
          modal: true,
          fixed: true,
          buttons: [
            ...
            },
            {
              ...
              class:
                "primary red-ui-palette-module-install-confirm-button-remove",
              click: function () {
                var spinner = RED.utils.addSpinnerOverlay(container, true);
                var buttonRow = $(
                  '<div style="position: relative;bottom: calc(50% + 17px); padding-right: 10px;text-align: right;"></div>'
                ...
                RED.eventLog.startEvent(
                  RED._("palette.editor.confirm.button.remove") +
                    " : " +
                    entry.name
                );
                removeNodeModule(entry.name, function (xhr) {
                  ...
                  if (xhr) {
                    if (xhr.responseJSON) {
                      var notification = RED.notify(
                        RED._("palette.editor.errors.removeFailed", {
                          module: entry.name,
                          message: xhr.responseJSON.message,
                        }),
                        {
    ...
    function install(entry, container, done) {
      ...
      var notification = RED.notify(
        RED._("palette.editor.confirm.install.body", { module: entry.id }),
        {
          modal: true,
          fixed: true,
          buttons: buttons,

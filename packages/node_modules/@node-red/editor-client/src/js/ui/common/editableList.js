 ...
 */
    $.widget( "nodered.editableList", {
            ...
            if (this.options.sortable) {
                ...
                var sortOptions = {
                    ...
                    update: function( event, ui ) {
                        console.log("IN UPDATE AT SORT OPTIONS")
                        console.log(that.options)
                        if (that.options.sortItems) {
                            console.log("that.options has sortItems")
        ...
        /* The above code appears to be a comment block in JavaScript. It includes a function or method
        called "_refreshSort" and a comment delimiter " */
        _refreshSort: function() {
            if (this.activeSort) {
                var items = this.element.children();
                var that = this;
                items.sort(function(A,B) {
                    return that.activeSort(
                        $(A).children(".red-ui-editableList-item-content").data('data'),
                        $(B).children(".red-ui-editableList-item-content").data('data')
                        );
                });
        ...
        getItemAt: function(index) {
            var items = this.items();
            console.log("items at getItemAt ", items)
            if (index >= 0 && index < items.length) {
                console.log("returning,", $(items[index]).data('data'))
                return $(items[index]).data('data');
            } else {
                console.log("returinng nothing")
                return;
            }
        },

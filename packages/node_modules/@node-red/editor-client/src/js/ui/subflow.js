 ...
 RED.subflow = (function () {
    ...
    function removeSubflowInput() {
      ...
      RED.nodes.eachLink(function (l) {
        if (
          l.source.type == "subflow" &&
          l.source.z == activeSubflow.id &&
    ...
    function removeSubflowOutput(removedSubflowOutputs) {
      ...
      for (i = 0; i < removedSubflowOutputs.length; i++) {
        ...
        RED.nodes.eachLink(function (l) {
          if (
            l.target.type == "subflow" &&
            l.target.z == activeSubflow.id &&
            l.target.i == output.i
          ) {
    ...
    function addSubflowStatus() {
      ...
      $("#red-ui-subflow-status")
        .parent()
        .parent()
        .toggleClass("active", !!subflow.status);
    }
    ...
    function refresh(markChange) {
      ...
      if (activeSubflow) {
        RED.nodes
          .filterNodes({ type: "subflow:" + activeSubflow.id })
          .forEach(function (n) {
    ...
    function refreshToolbar(activeSubflow) {
      if (activeSubflow) {
        $("#red-ui-subflow-input-add").toggleClass(
          "active",
          activeSubflow.in.length !== 0
        );
        $("#red-ui-subflow-input-remove").toggleClass(
          "active",
          activeSubflow.in.length === 0
        );
        ...
        $("#red-ui-subflow-status")
          .parent()
          .parent()
          .toggleClass("active", !!activeSubflow.status);
      }
    }
    ...
    function showWorkspaceToolbar(activeSubflow) {
      ...
      // Edit properties
      $(
        '<a class="button" id="red-ui-subflow-edit" href="#" data-i18n="[append]subflow.editSubflowProperties"><i class="fa fa-pencil"></i> </a>'
      ).appendTo(toolbar);
      ...
      // Status
      $(
        '<span class="button-group"><span class="button" style="padding:0"><label for="red-ui-subflow-status"><input id="red-ui-subflow-status" type="checkbox"> <span data-i18n="subflow.status"></span></label></span></span>'
      ).appendTo(toolbar);
      ...
      // Delete
      $(
        '<a class="button" id="red-ui-subflow-delete" href="#" data-i18n="[append]subflow.deleteSubflow"><i class="fa fa-trash"></i> </a>'
      ).appendTo(toolbar);
      ...
      $("#red-ui-subflow-output-remove").on("click", function (event) {
        ...
        var wasDirty = RED.nodes.dirty();
        if(RED.nodes.subflow(RED.workspaces.active())){
        ...
        }else if(RED.nodes.submodel(RED.workspaces.active())){
          var wasChanged = activeSubflow.changed;
          var result = RED.submodel.removeOutput();
          if (result) {
            var inst = refresh(true);
            RED.history.push({
              t: "delete",
      ...
      $("#red-ui-subflow-output-add").on("click", function (event) {
        ...
        if(RED.nodes.subflow(RED.workspaces.active())){
          ...
          RED.submodel.addSubmodelOutput();
        }
      });
      ...
      $("#red-ui-subflow-input-remove").on("click", function (event) {
        ...
        if(RED.nodes.subflow(RED.workspaces.active())){
          ...
          var wasChanged = activeSubflow.changed;
          activeSubflow.changed = true;
          var result = RED.submodel.removeInput();
          if (result) {
            var inst = refresh(true);
            RED.history.push({
              t: "delete",
      ...
      $("#red-ui-subflow-status").on("change", function (evt) {
        ...
        if(RED.nodes.submodel(RED.workspaces.active())){
          if (this.checked) {
            RED.submodel.addSubmodelStatus();
          } else {
            // keep activeSubflow since its parameter being passed in -- represents either activeSubflow or activeSubmodel
            var currentStatus = activeSubflow.status;
            var wasChanged = activeSubflow.changed;
            var result = RED.submodel.removeStatus();
            if (result) {
              activeSubflow.changed = true;
              var wasDirty = RED.nodes.dirty();
              RED.history.push({
      ...
      $("#red-ui-subflow-delete").on("click", function (event) {
        ...
        if(subflow){
          console.log("in subflow : instances ", subflow.instances)
          if (subflow.instances.length > 0) {
            var msg = $("<div>");
            $("<p>")
              .text(
                RED._("subflow.subflowInstances", {
                  count: subflow.instances.length,
                })
              )
              .appendTo(msg);
        ...
        if(submodel){
          console.log("in submodel : instances " , submodel.instances)
          ...
          refreshToolbar(submodel);

        }
  
        function completeDelete(type) {
          ...
          if(type === "subflow"){
            ...
            var historyEvent = RED.submodel.removeSubmodel(RED.workspaces.active());
            historyEvent.t = "delete";
            historyEvent.dirty = startDirty;
            RED.history.push(historyEvent);
          }
    ...
    function buildEnvEditRow(container, ui, nameField, valueField) {
      ...
      var labelIcon = $(
        '<span class="red-ui-editor-subflow-env-lang-icon"><i class="fa fa-language"></i></span>'
      ).appendTo(labelCell);
          ...
          types: [
            {
              ...
              valueLabel: function (container, value) {
                ...
                var innerContainer = $(
                  '<div class="red-ui-editor-subflow-env-input-type"></div>'
                ).appendTo(container);
                ...
                if (value.length) {
                  ...
                  $(
                    '<span class="red-ui-editor-subflow-env-input-type-placeholder"></span>'
                  )
            ...
            },
            {
              ...
              valueLabel: function (container, value) {
                container.css("padding", 0);
                var innerContainer = $(
                  '<div class="red-ui-editor-subflow-env-input-type">'
                )
                  .css({
            ...
            },
            {
              ...
              expand: {
                ...
                content: function (container) {
                  var content = $(
                    '<div class="red-ui-editor-subflow-ui-edit-panel">'
                  ).appendTo(container);
                  var optList = $("<ol>")
                    .appendTo(content)
                    .editableList({
    ...
    function getSubflowInstanceParentEnv(node) {
      ...
      var envList = [];
      if (/^subflow:/.test(node.type)) {
        console.log(node.type.substring(8))
        var subflowDef = RED.nodes.subflow(node.type.substring(8));
        if (subflowDef.env) {
          subflowDef.env.forEach(function (env) {
            var item = {
    ...
    function getSubflowEnvPropertyName(name) {
      return "node-input-subflow-env-" + name.replace(/[^a-z0-9-_]/gi, "_");
    }
    ...
    function buildEditForm(type, node) {
      if (type === "subflow-template") {
        ...
        buildEnvUI(
          $("#subflow-input-ui"),
          getSubflowInstanceParentEnv(node),
          node
        );
      }
    }
    function buildPropertiesForm(node) {
      ...
      var list = $(
        '<ol id="red-ui-editor-subflow-env-list" class="red-ui-editor-subflow-env-list"></ol>'
    ...
    function exportSubflowModuleProperties(node) {
      ...
      ["module", "type", "version", "author", "desc", "keywords"].forEach(
        function (property) {
          value = $("#subflow-input-module-" + property)
            .val()
            .trim();
          if (value) {
      ...
      var selectedLicenseType = $("#subflow-input-module-license").typedInput(
        "type"
      );

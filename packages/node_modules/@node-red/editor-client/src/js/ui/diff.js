RED.diff = (function() {
    ...
    function buildDiffPanel(container,diff,options) {
        ...
        return {
            ...
            finish: function() {
                ...
                var submodelId;
                for (submodelId in currentConfig.submodels) {
                    if (currentConfig.submodels.hasOwnProperty(submodelId)) {
                        seenTabs[submodelId] = true;
                        el = {
                            diff: localDiff,
                            def: {
                                defaults:{},
                            ...
                            tab:currentConfig.submodels[submodelId]
                        }
                        if (newConfig.submodels.hasOwnProperty(submodelId)) {
                            el.newTab = newConfig.submodels[submodelId];
                        }
                        if (remoteDiff !== undefined) {
                            el.remoteTab = remoteDiff.newConfig.submodels[submodelId];
                            el.remoteDiff = remoteDiff;
                        }
                        diffList.editableList('addItem',el)
                    }
                }
                for (submodelId in newConfig.submodels) {
                    if (newConfig.submodels.hasOwnProperty(submodelId) && !seenTabs[submodelId]) {
                        seenTabs[submodelId] = true;
                        el = {
                            diff: localDiff,
                            def: {
                            ...
                            tab:newConfig.submodels[submodelId],
                            newTab:newConfig.submodels[submodelId]
                        }
                        if (remoteDiff !== undefined) {
                            el.remoteDiff = remoteDiff;
                        }
                ...
                if (remoteDiff !== undefined) {
                    for (submodelId in remoteDiff.newConfig.submodels) {
                        if (remoteDiff.newConfig.submodels.hasOwnProperty(submodelId) && !seenTabs[submodelId]) {
                            el = {
                                diff: localDiff,
                                remoteDiff: remoteDiff,
                                def: {
                                ...
                                tab:remoteDiff.newConfig.submodels[submodelId],
                                remoteTab: remoteDiff.newConfig.submodels[submodelId]
                            }
                            diffList.editableList('addItem',el)
                        }
                    }
    ...
    function createNodeConflictRadioBoxes(node,row,localDiv,remoteDiv,propertiesTable,hide,state,diff) {
        ...
        var titleRow = !propertiesTable && (node.type === 'tab' || node.type === 'subflow' || node.type ==='submodel');
    ...
    function parseNodes(nodeList) {
        ...
        nodeList.forEach(function(node) {
            ...
            if (node.type === 'tab') {
                ...
                submodels[node.id] = {n: node, nodes: []};
            }
        });
        ...
        nodeList.forEach(function(node) {
            if (node.type !== 'tab' && node.type !== 'subflow' && node.type !== 'submodel') {
                ...
                } else if (submodels[node.z]){
                    submodels[node.z].nodes.push(node);
                }else {
                    globals.push(node);
                }
            }
    ...
    function generateDiff(currentNodes,newNodes) {
        ...
        Object.keys(currentConfig.all).forEach(function(id) {
            var node = RED.nodes.workspace(id)||RED.nodes.subflow(id)|| RED.nodes.submodel(id) || RED.nodes.node(id);

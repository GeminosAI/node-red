...
RED.sidebar.help = (function () {
    ...
    function refreshSubmodel(sf) {
        var item = treeList.treeList('get', "node-type:submodel:" + sf.id);
        item.submodelLabel = sf._def.label().toLowerCase();
        item.treeList.replaceElement(getNodeLabel({ _def: sf._def, type: sf._def.label() }));
    }
    ...
    function refreshHelpIndex() {
        ...
        var submodels = RED.nodes.registry.getNodeTypes().filter(function (t) { return /submodel/.test(t) });
        ...
        if (submodels.length > 0) {
            nodeHelp.children.push({
                label: RED._("menu.label.submodels"),
                children: []
            })
            submodels.forEach(function (nodeType) {
                var sf = RED.nodes.getType(nodeType);
                nodeHelp.children[0].children.push({
                    id: "node-type:" + nodeType,
                    nodeType: nodeType,
                    submodelLabel: sf.label().toLowerCase(),
                    element: getNodeLabel({ _def: sf, type: sf.label() })
                })
            })
        }
    ...
    function showNodeTypeHelp(nodeType) {
        ...
        if (m && m[2]) {
            ...
            var submodelNode = RED.nodes.submodel(n[2]);
            helpText = (RED.utils.renderMarkdown(submodelNode.info || "") || ('<span class="red-ui-help-info-none">' + RED._("sidebar.info.none") + '</span>'));
            title = submodelNode.name || nodeType;
        } else {

...
RED.group = (function() {
    ...
    var _groupEditTemplate = '<script type="text/x-red" data-template-name="group">'+
        ...
        '<div class="form-row">'+
            '<span>'+
                '<label for= "node-input-abstract" > Abstract Class </label>'+
                '<input type="checkbox" id="node-input-abstract" style="width:5%;" checked/>'+
            '</span>'+
        '</div>'+
        '<div class="form-row">'+
            '<span>'+
                '<label for="node-input-classtype"> Class Type </label>' +
        ...
        '<div class="form-row">' +
            '<span>'+
                ...
                '<input type="text" id="node-input-key" style="width:35%; display:inline">'+
                '</input>'+
                '<div style = "display:inline; padding: 0 0 0 12px"> type: </div>'+
                '<select id="node-input-keytype" style="width:20%; display:inline">'+
        ...
        '<div class="form-row">'+
            '<div>'+
                '<ul id="node-input-property-tabs"></ul>'+
                '</div>'+
            '</div>'+
        '<div id="node-input-property-tabs-content" class="form-row">'+
            '<div id="node-input-container-owns">'+
            '<ol id="node-input-property-container-owns"></ol>'+
            '</div>'+
            '<div id="node-input-container-relates">'+
            '<ol id="node-input-property-container-relates"></ol>'+
            '</div>'+
            '<div id="node-input-container-plays">'+
            '<ol id="node-input-property-container-plays"></ol>'+
            '</div>'+
        '</div>'+
    '</script>';
    ...
    var groupDef = {
        defaults:{
            ...
            classtype: {value: "entity"},
            abstract: {value: true},
            key: {value: ""},
            keytype: {value: $("#node-input-keytype")},
            addedDefaults:{value:false}
        },
        category: "config",
        oneditprepare: function() {
            console.log("Properties of a group node: ", groupDef.defaults.props  )
            this.props = groupDef.defaults.props.value
            ...
            var tabs = RED.tabs.create({
                id: "node-input-property-tabs",
                vertical: false,
                onchange: function (tab) {
                  $("#node-input-property-tabs-content").children().hide();
                  $("#" + tab.id).show();
                  activeTab = tab.id;
                },
              });
              tabs.addTab({
                id: "node-input-container-owns",
                label: "Owns",
              });
              tabs.addTab({
                id: "node-input-container-relates",
                label: "Relates",
              });
              tabs.addTab({
                id: "node-input-container-plays",
                label: "Plays",
              });
        
            $("#node-input-abstract").on("click", function(e)
            {
                console.log($(this).is(":checked"))
            })
            $("#node-input-property-container-owns").css("min-height", "375px").css("max-height", "750px").css("min-width","450px").editableList({
                addItem:function(container, i , opt){
                    var prop = opt;
                    if (!prop.hasOwnProperty("p")) {
                      prop = { p: "owns", v: "", vt: "string", vtindex: 0, key: false};
                    }
        
                    // if index property doesnt exists set to 0
                    if (!opt.hasOwnProperty("i")) {
                      opt._i = Math.floor(
                        (0x99999 - 0x10000) * Math.random()
                      ).toString();
                      // console.log("Property Index: ", opt._i);
                    }
                    ...
                    // Separator
                    var textClassName = "node-input-prop-property-typetext";
                    $("<div/>", {
                    ...
                    // Selecting datatype for attribute
                    var className = "node-input-prop-property-datatype" + i;
                    var propertyType = $("<select/>", {
                      class: className,
                      type: "text",
                    })
                      .css("width", "30%")
                    ...
                    if (prop.vt) {
                      var key_index = prop.vtindex;
                      propertyType.append(
                        $("<option />", {
                          value: key_index,
                          text: grakn_datatypes[key_index],
                        })
                      );
                      $.each(grakn_datatypes, function (key, value) {
                        if (key !== key_index) {
                          propertyType.append(
                            $("<option />", {
                              value: key,
                    ...
                    // append the grakn datatype option field
                    else {
                      $.each(grakn_datatypes, function (key, value) {
                        propertyType.append(
                          $("<option />", {
                    ...
                    propertyType.on("change", function () {
                      prop.vt = grakn_datatypes[propertyType.val()];
                      prop.vtindex = propertyType.val();
            ...
            $("#node-input-property-container-relates")
            .css("min-height", "375px")
            .css("max-height", "750px")
            .css("min-width", "400px")
            .editableList({
              addItem: function (container, i, opt) {
                var prop = opt;
                console.log("Property when initializing : ", prop);
                if (!prop.hasOwnProperty("p")) {
                  prop = { p: "relates", v: "", vt: "string", vtindex: 0 };
                }
            ...
            $("#node-input-property-container-plays").css("min-height", "375px").css("max-height", "550px").css("min-width","450px").editableList({
                addItem:function(container, i , opt){
                    var prop = opt;
                    if (!prop.hasOwnProperty("p")) {
                      prop = { p: "plays", v: "", vt: "string", vtindex: 0 };
                    }
            ...
            $("#node-input-classtype").trigger("change");
            $("#node-input-key").trigger("change");
            $("#node-input-keytype").trigger("change");
            ...
            for (var i = 0; i < this.props.length; i++) {
                ...
                console.log("adding prop of  ", prop);
                var newProp = {
                  p: prop.p,
                  v: prop.v,
                  vt: prop.vt,
                  vtindex: prop.vtindex,
                };
                if (newProp.p === "owns") {
                  $("#node-input-property-container-owns").editableList(
                    "addItem",
                    newProp
                  );
                }
                if (newProp.p === "relates") {
                  $("#node-input-property-container-relates").editableList(
                    "addItem",
                    newProp
                  );
                }
                if (newProp.p === "plays") {
                  $("#node-input-property-container-plays").editableList(
                    "addItem",
                    newProp
                  );
                }
        ...
        oneditsave: function() {
            ...
            // grab properties in containers
            var props = $("#node-input-property-container-owns").editableList(
                "items"
            );
            var relates_props =  $("#node-input-property-container-relates").editableList(
                "items"
                );
            var play_props = $("#node-input-property-container-plays").editableList(
                "items"
            );
            ...
            props.each(function (i) {
                ...
                p.v = prop.find(".node-input-prop-property-value").val();
                var className = ".node-input-prop-property-datatype" + i;
                p.vtindex = prop.find(className).val();
                p.vt = grakn_datatypes[p.vtindex];
                node.props.push(p);
            });
            play_props.each(function () {
                var prop = $(this);
                var p = {
                p: "plays",
                };
    ...
    function groupSelection() {
        if (RED.view.state() !== RED.state.DEFAULT) { return }
        var selection = RED.view.selection();
        console.log("red view groupSelection : ", selection)
        if (selection.nodes) {
            console.log("IN GROUP SELECTION")

...
RED.view = (function () {
    ...
    function init() {
        ...
        RED.events.on("workspace:change", function (event) {
            console.log("@emit workspace change ")
            if (event.old !== 0) {
            ...
            if (activeSubmodel) {
                RED.menu.setDisabled("menu-item-workspace-edit", activeSubmodel || event.workspace === 0);
                RED.menu.setDisabled("menu-item-workspace-delete", RED.workspaces.count() == 1 || activeSubmodel);
            }
    ...
    function addNode(type, x, y) {
        ...
        if (activeSubmodel && c) {
            ...
            if (submodelId === activeSubmodel.id) {
                RED.notify(RED._("notification.error", { message: RED._("notification.errors.cannotAddSubmodelToItself") }), "error");
                return;
            }
        ...
        if (workspaceSelection.length > 0) {
            ...
            if (movingSet.length() > 0) {
                ...
                for (var i = 0; i < movingSet.length(); i++) {
                    node = movingSet.get(i).n;
                    node.selected = false;
                    if (node.type !== "group" && node.type !== "subflow" && node.type !== "submodel") {
                        if (node.x < 0) {
                            node.x = 25
                        }
                ...
                if (removedNodes.length > 0 || removedSubmodelOutputs.length > 0 || removedSubmodelInputs.length > 0 || removedSubmodelStatus || removedGroups.length > 0) {
                    RED.nodes.dirty(true);
                }
            }
            ...
            if (instances) {
                historyEvent.subflowOutputs = removedSubflowOutputs,
                    historyEvent.subflowInputs = removedSubflowInputs,
                    historyEvent.subflow = {
    ...
    function groupMouseDown(g) {
        ...
        if (g.selected && (d3.event.ctrlKey || d3.event.metaKey)) {
            ...
            if (!g.selected) {
                ...
                // add selected nodes to variable movingSet
    ...
    function _redraw() {
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            linkEnter.each(function (d, i) {
                ...
                // geminos
                if (d.source.type === "entity" || d.source.type === "relation" || d.source.type === "causeeffect" || /^submodel:(.+)$/.exec(d.source.type) || d.source.type === "attribute") {
                ...
                if ((d.source.type === "causeeffect" || /^submodel:(.+)$/.exec(d.source.type)) && (d.target.type === "causeeffect" || /^submodel:(.+)$/.exec(d.target.type))) {
                    ...
                    pathText.textContent = "cause-output";
                    pathText.setAttribute("startOffset", "50%");
                    pathContents.appendChild(text);
                }
    ...
    function importNodes(newNodesObj, options) {
        ...
        console.log("newNodesObj: ", newNodesObj);
        ...
        console.log("nodesToImport ", nodesToImport);
        try {
            var activeSubflowChanged;
            console.log("activeSubflow", activeSubflow);
            console.log("activeSubmodel", activeSubmodel);
            if (activeSubflow) {
            ...
            console.log("result in view.js ", result)
            if (result) {

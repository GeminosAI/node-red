...
RED.view = (function () {
            ...
            add: function (node) {
                if (Array.isArray(node)) {
                    ...
                    if (!setIds.has(node.id)) {
                        set.push({ n: node });
                        setIds.add(node.id);
                        console.log(PORT_TYPE_INPUT)
                        console.log(PORT_TYPE_OUTPUT)
    ...
    function init() {
        ...
        RED.events.on("workspace:change", function (event) {
            ...
            activeSubmodel = RED.nodes.submodel(event.workspace)

            if (activeSubflow) {
                activeFlowLocked = activeSubflow.locked
            } else if (activeSubmodel) {
                activeFlowLocked = activeSubmodel.locked
            }
            else {
            ...
            RED.menu.setDisabled("menu-item-workspace-edit", activeFlowLocked || activeSubflow || activeSubmodel || event.workspace === 0);
            RED.menu.setDisabled("menu-item-workspace-delete", activeFlowLocked || event.workspace === 0 || RED.workspaces.count() == 1 || activeSubflow || activeSubmodel);
        ...
        chart.droppable({
            ...
            drop: function (event, ui) {
                ...
                try {
                    var isLink = (nn.type === "link in" || nn.type === "link out" || nn.type === "probability" || nn.type === "moderator")
                    var hideLabel = nn.hasOwnProperty('l') ? !nn.l : isLink;
                    if (nn.type === "probability" || nn.type === "moderator") { hideLabel = true }
                    console.log("hide label; ", hideLabel)
                    var label = RED.utils.getNodeLabel(nn, nn.type);
                    console.log("label: ", label)
                    var labelParts = getLabelParts(label, "red-ui-flow-node-label");
                    if (hideLabel) {
                        console.log("in hide label")
                        nn.w = node_height;
                        nn.h = Math.max(node_height, (nn.outputs || 0) * 15);
                        console.log(nn.w, nn.h)
                    } else {
                ...
                console.log("nn.x: ", nn.x)
                console.log("nn.y: ", nn.y)
                var minX = nn.w / 2 - 5;
                if (nn.x < minX) {
                    nn.x = minX;
                }
        ...
        RED.actions.add("core:go-to-selection", function () {
            if (movingSet.length() > 0) {
                ...
                } else if (/^submodel:/.test(node.type)) {
                    RED.workspaces.show(node.type.substring(9))
    ...
    function showQuickAddDialog(options) {
        ...
        RED.typeSearch.show({
            ...
            add: function (type, keepAdding) {
                ...
                console.log("ON ADD SHOW LABEL", showLabel)
                if (nn.type === 'junction') {
    ...
    function canvasMouseUp() {
        ...
        if (lasso) {
            ...
            // geminos
            if (activeSubmodel) {
                activeSubmodel.in.forEach(function (n) {
                    n.selected = (n.x > x && n.x < x2 && n.y > y && n.y < y2);
                    if (n.selected) {
                        n.dirty = true;
                        movingSet.add(n);
                ...
                activeSubmodel.out.forEach(function (n) {
                    n.selected = (n.x > x && n.x < x2 && n.y > y && n.y < y2);
                    if (n.selected) {
                        n.dirty = true;
                        movingSet.add(n);
                ...
                if (activeSubmodel.status) {
                    activeSubmodel.status.selected = (activeSubmodel.status.x > x && activeSubmodel.status.x < x2 && activeSubmodel.status.y > y && activeSubmodel.status.y < y2);
                    if (activeSubmodel.status.selected) {
                        activeSubmodel.status.dirty = true;
                        movingSet.add(activeSubmodel.status);
                    }
                }
            }
            updateSelection();
    ...
    function selectAll() {
        ...
        // geminos
        if (mouse_mode !== RED.state.SELECTING_NODE && activeSubmodel) {
            activeSubmodel.in.forEach(function (n) {
                if (!n.selected) {
                    n.selected = true;
                    n.dirty = true;
            ...
            if (activeSubmodel.status) {
                if (!activeSubmodel.status.selected) {
                    activeSubmodel.status.selected = true;
                    activeSubmodel.status.dirty = true;
                    movingSet.add(activeSubmodel.status);
                }
            }
    ...
    function editSelection() {
        ...
        if (movingSet.length() > 0) {
            ...
            if (node.type === "subflow") {
                ...
                RED.editor.editSubmode(activeSubmodel)
            } else if (node.type === "group") {
    ...
    function deleteSelection(reconnectWires) {
        ...
        if (workspaceSelection.length > 0) {
            ...
            for (var i = 0; i < workspaceSelection.length; i++) {
                ...
                } else if (ws.type === "subflow") {
                    subEvent = RED.subflow.removeSubflow(ws.id);
                    historyEvent.subflows = historyEvent.subflows.concat(subEvent.subflows);
                } else if (ws.type === "submodel") {
                    subEvent = RED.submodel.removeSubmodel(ws.id);
                    historyEvent.submodels = historyEvent.submodels.concat(subEvent.submodels);
                }
            ...
            var removedSubmodelOutputs = [];
            var removedSubmodelInputs = [];
            var removedSubmodelStatus;
            ...
            if (movingSet.length() > 0) {
                ...
                for (var i = 0; i < movingSet.length(); i++) {
                    node = movingSet.get(i).n;
                    node.selected = false;
                    if (node.type !== "group" && node.type !== "subflow" && node.type !== "submodel" && node.type !== 'junction') {
                        if (node.x < 0) {
                            node.x = 25
                        }
                        ...
                        if (node.type === " submodel") {
                            if (node.direction === "out") {
                                removedSubmodelOutputs.push(node);
                            } else if (node.direction === "in") {
                                removedSubmodelInputs.push(node);
                            } else if (node.direction === "status") {
                                removedSubmodelStatus = node;
                            }
                        }
                        node.dirty = true;
                    }
                ...
                // GEM Assume 0/1 inputs
                if (removedSubmodelInputs.length == 1) {
                    result = RED.submodel.removeInput();
                    if (result) {
                        removedLinks = removedLinks.concat(result.links);
                    }
                }
                if (removedSubmodelStatus) {
                    result = RED.submodel.removeStatus();
                    if (result) {
                        removedLinks = removedLinks.concat(result.links);
                    }
                }
                if (removedSubmodelOutputs.length > 0) {
                    result = RED.submodel.removeOutput(removedSubmodelOutputs);
                    if (result) {
                        removedLinks = removedLinks.concat(result.links);
                    }
                }
                ...
                var submodel_instances = RED.submodel.refresh(true);

                if (submodel_instances) {
                    submodelInstances = submodel_instances.instances;
                }
                movingSet.clear();
                if (removedNodes.length > 0 || removedSubflowOutputs.length > 0 || removedSubflowInputs.length > 0 || removedSubflowStatus || removedGroups.length > 0 || removedJunctions.length > 0 || removedSubmodelOutputs.length > 0 || removedSubmodelInputs.length > 0 || removedSubmodelStatus) {
                    RED.nodes.dirty(true);
                }
            }
            ...
            console.log("deleting node here")
            RED.nodes.dirty(true);
    ...
    function copySelection(isCut) {
        ...
        if (selection.length > 0) {
            ...
            console.log("red. view. selection ", selection)
            if (selection.nodes) {
        ...
        if (nodes.length > 0) {
            ...
            for (var n = 0; n < nodes.length; n++) {
                ...
                if (node.type !== "subflow" && node.type !== "submodel") {
                    if (node.type === "group") {
                        groupCount++;
                    } else if (node.type === 'junction') {
                        junctionCount++;
    ...
    function portMouseUp(d, portType, portIndex, evt) {
        ...
        if (mouse_mode == RED.state.JOINING || mouse_mode == RED.state.QUICK_JOINING) {
            if (typeof TouchEvent != "undefined" && evt instanceof TouchEvent) {
                ...
                if (!found && activeSubmodel) {
                    var submodelPorts = [];
                    if (activeSubmodel.status) {
                        submodelPorts.push(activeSubmodel.status)
                    }
                    if (activeSubmodel.in) {
                        submodelPorts = submodelPorts.concat(activeSubmodel.in)
                    }
                    if (activeSubmodel.out) {
                        submodelPorts = submodelPorts.concat(activeSubmodel.out)
                    }
                    for (var i = 0; i < submodelPorts.length; i++) {
                        var n = submodelPorts[i];
                        var hw = n.w / 2;
                        var hh = n.h / 2;
                ...
                if (portType != drag_lines[i].portType && mouseup_node !== drag_lines[i].node) {
                    ...
                    if (drag_line.virtualLink) {
                        ...
                        if (!(
                            ...
                            (portType === PORT_TYPE_OUTPUT && mouseup_node.type !== "subflow" && mouseup_node.type !== "submodel" && mouseup_node.outputs === 0) ||
                            (portType === PORT_TYPE_INPUT && mouseup_node.type !== "subflow" && mouseup_node.type !== "submodel" && mouseup_node.inputs === 0) ||
                            ...
                            (drag_line.portType === PORT_TYPE_OUTPUT && mouseup_node.type === "submodel" && (mouseup_node.direction === "status" || mouseup_node.direction === "out")) ||
                            (drag_line.portType === PORT_TYPE_OUTPUT && mouseup_node.type === "submodel" && mouseup_node.direction === "in")
                        )) {
            ...
            if (addedLinks.length > 0 || removedLinks.length > 0 || modifiedNodes.length > 0) {
                ...
                if (activeSubmodel) {
                    var submodelRefresh = RED.submodel.refresh(true);
                    if (submodelRefresh) {
                        historyEvent.submodel = {
                            id: activeSubmodel.id,
                            changed: activeSubmodel.changed,
                            instances: submodelRefresh.instances
                        }
                    }
                }
                RED.history.push(historyEvent);
    ...
    function nodeMouseUp(d) {
        ...
        if (dblClickPrimed && mousedown_node == d && clickElapsed > 0 && clickElapsed < dblClickInterval) {
            ...
            if (d.type != "subflow" || d.type != "submodel") {
                if (/^subflow:/.test(d.type) && (d3.event.ctrlKey || d3.event.metaKey)) {
                    RED.workspaces.show(d.type.substring(8));
                } else if (/^submodel:/.test(d.type) && (d3.event.ctrlKey || d3.event.metaKey)) {
                    RED.workspaces.show(d.type.substring(9));
                } else {
                    RED.editor.edit(d);
                }
            } else {
    ...
    function showTouchMenu(obj, pos) {
        ...
        options.push({
            name: "add", disabled: isActiveLocked, onselect: function () {
    ...
    async function _redraw() {
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            // GEM
            if (activeSubmodel) {
                var submodelOutputs = nodeLayer.selectAll(".red-ui-flow-submodel-port-output").data(activeSubmodel.out, function (d, i) { return d.id; });
                submodelOutputs.exit().remove();
                var outGroup = submodelOutputs.enter().insert("svg:g").attr("class", "red-ui-flow-node red-ui-flow-submodel-port-output").attr("transform", function (d) { return "translate(" + (d.x - 20) + "," + (d.y - 20) + ")" });
                outGroup.each(function (d, i) {
                    d.w = 40;
                    d.h = 40;
                });
                outGroup.append("rect").attr("class", "red-ui-flow-submodel-port").attr("rx", 8).attr("ry", 8).attr("width", 40).attr("height", 40)
                    // TODO: This is exactly the same set of handlers used for regular nodes - DRY
                    .on("mouseup", nodeMouseUp)
                    .on("mousedown", nodeMouseDown)
                    .on("touchstart", nodeTouchStart)
                ...
                var submodelInputs = nodeLayer.selectAll(".red-ui-flow-submodel-port-input").data(activeSubmodel.in, function (d, i) { return d.id; });
                submodelInputs.exit().remove();
                var inGroup = submodelInputs.enter().insert("svg:g").attr("class", "red-ui-flow-node red-ui-flow-submodel-port-input").attr("transform", function (d) { return "translate(" + (d.x - 20) + "," + (d.y - 20) + ")" });
                inGroup.each(function (d, i) {
                    d.w = 40;
                    d.h = 40;
                });
                inGroup.append("rect").attr("class", "red-ui-flow-submodel-port").attr("rx", 8).attr("ry", 8).attr("width", 40).attr("height", 40)
                    // TODO: This is exactly the same set of handlers used for regular nodes - DRY
                    .on("mouseup", nodeMouseUp)
                    .on("mousedown", nodeMouseDown)
                    .on("touchstart", nodeTouchStart)
                ...
                var submodelStatus = nodeLayer.selectAll(".red-ui-flow-submodel-port-status").data(activeSubmodel.status ? [activeSubmodel.status] : [], function (d, i) { return d.id; });
                submodelStatus.exit().remove();

                var statusGroup = submodelStatus.enter().insert("svg:g").attr("class", "red-ui-flow-node red-ui-flow-submodel-port-status").attr("transform", function (d) { return "translate(" + (d.x - 20) + "," + (d.y - 20) + ")" });
                statusGroup.each(function (d, i) {
                    d.w = 40;
                    d.h = 40;
                });
                statusGroup.append("rect").attr("class", "red-ui-flow-submodel-port").attr("rx", 8).attr("ry", 8).attr("width", 40).attr("height", 40)
                    // TODO: This is exactly the same set of handlers used for regular nodes - DRY
                    .on("mouseup", nodeMouseUp)
                    .on("mousedown", nodeMouseDown)
                    .on("touchstart", nodeTouchStart)
                ...
                submodelOutputs.each(function (d, i) {
                    if (d.dirty) {
                        var output = d3.select(this);
                        output.classed("red-ui-flow-node-selected", function (d) { return d.selected; })
                        output.selectAll(".red-ui-flow-port-index").text(function (d) { return d.i + 1 });
                ...
                submodelInputs.each(function (d, i) {
                    if (d.dirty) {
                        var input = d3.select(this);
                        input.classed("red-ui-flow-node-selected", function (d) { return d.selected; })
                        input.attr("transform", function (d) { return "translate(" + (d.x - d.w / 2) + "," + (d.y - d.h / 2) + ")"; });
                ...
                submodelStatus.each(function (d, i) {
                    if (d.dirty) {
                        var output = d3.select(this);
                        output.classed("red-ui-flow-node-selected", function (d) { return d.selected; })
                        output.selectAll(".red-ui-flow-port-index").text(function (d) { return d.i + 1 });
                ...
                nodeLayer.selectAll(".red-ui-flow-submodel-port-output").remove();
                nodeLayer.selectAll(".red-ui-flow-submodel-port-input").remove();
                nodeLayer.selectAll(".red-ui-flow-submodel-port-status").remove();
            }
            ...
            var nodeEnter;
            if (activeSubmodel) {
                nodeEnter = node.enter().insert("svg:g")
                    .attr("class", "red-ui-flow-node red-ui-flow-node-group")
                    .classed("red-ui-flow-submodel", activeSubmodel != null);
            } else {
                nodeEnter = node.enter().insert("svg:g")
                    .attr("class", "red-ui-flow-node red-ui-flow-node-group")
            ...
            nodeEnter.each(function (d, i) {
                ...
                if (d.type === "probability" || d.type === "moderator") {
                    hideLabel = true
                }
                node.attr("id", "_" + d.id);
                d.h = node_height;
                ...
                mainRect.setAttribute("rx", 7);
                mainRect.setAttribute("ry", 7);
                ...
                if (d.dirty) {
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        ...
                        if ((!isLink || (showAllLinkPorts === -1 && !activeLinkNodes[d.id])) && d.inputs === 0 && !inputPorts.empty()) {
                            ...
                            var inputGroupPorts;
                            if (d.type === "link in") {
                                inputGroupPorts = inputGroup.append("circle").attr("cx", -1).attr("cy", 5).attr("r", 5).attr("class", "red-ui-flow-port red-ui-flow-link-port")
                            } else {
                                inputGroupPorts = inputGroup.append("rect").attr("class", "red-ui-flow-port").attr("rx", 3).attr("ry", 3).attr("width", 10).attr("height", 10)
                            }
                            // console.log("input group " , inputGroup[0][0])
                            // console.log(inputGroupPorts[0][0])
                            // console.log("updating input group")
            ...
            let result = await RED.causeway.getBackdoors()
            let backdoors = result[0]
            let backdoorKeys = new Set(Object.keys(backdoors))
            let frontdoors = result[1]
            let frontdoorKeys = new Set(Object.keys(frontdoors))
            let open_paths = result[2]
            let closed_paths = result[3]


            // GEMINOS: added section
            linkLayer.selectAll(".red-ui-closed-causal-link-line").attr("class",
                function (d, i) {
                    return "red-ui-flow-link-line red-ui-flow-link-path" +
                        (d.link ? " red-ui-flow-link-link" : (activeSubflow ? " red-ui-flow-subflow-link" : "")) +
                        (d.source.d || d.target.d ? "red-ui-flow-node-disabled" : "");
                })
            linkLayer.selectAll(".red-ui-open-causal-link-line").attr("class",
                function (d, i) {
                    return "red-ui-flow-link-line red-ui-flow-link-path" +
                        (d.link ? " red-ui-flow-link-link" : (activeSubflow ? " red-ui-flow-subflow-link" : "")) +
                        (d.source.d || d.target.d ? "red-ui-flow-node-disabled" : "");
            ...
            // GEMINOS eof added section 
            if (backdoorKeys.size > 0 || frontdoorKeys.size > 0) {
                linkLayer.selectAll(".red-ui-flow-link-line").attr("class",
                    function (d, i) {
                        // GEMINOS: added section 
                        let pathId = d.source.id + "," + d.target.id
                        if (backdoorKeys.has(pathId)) {
                            let linkType = backdoors[pathId]["type"] ? "red-ui-closed-causal-link-line" : "red-ui-open-causal-link-line"
                            return linkType + " red-ui-flow-link-path" +
                                (d.link ? " red-ui-flow-link-link" : (activeSubflow ? " red-ui-flow-subflow-link" : "")) +
                                (d.source.d || d.target.d ? " red-ui-flow-node-disabled" : "");

                        }
                        if (frontdoorKeys.has(pathId)) {
                            let linkType = "red-ui-open-causal-link-line"
                            return linkType + " red-ui-flow-link-path" +
                                (d.link ? " red-ui-flow-link-link" : (activeSubflow ? " red-ui-flow-subflow-link" : "")) +
                                (d.source.d || d.target.d ? " red-ui-flow-node-disabled" : "");
                            // GEMINOS eof added section 
            ...
            linkEnter.each(async function (d, i) {
                var l = d3.select(this);
                var pathContents = document.createDocumentFragment();
                var pathText = document.createElementNS("http://www.w3.org/2000/svg", 'textPath')
                ...
                var markerArrowR = document.createElementNS("http://www.w3.org/2000/svg", "marker")
                var arrowR = document.createElementNS("http://www.w3.org/2000/svg", "path")
                var arrowL = document.createElementNS("http://www.w3.org/2000/svg", "path")
                var markerArrowL = document.createElementNS("http://www.w3.org/2000/svg", "marker")
                var filter = document.createElementNS("http://www.w3.org/2000/svg", 'filter')
                filter.setAttribute("id", "text-rect")
                var filterFlood = document.createElementNS("http://www.w3.org/2000/svg", 'feFlood')
                filterFlood.setAttribute("flood-color", "gray")
                filterFlood.setAttribute("flood-opacity", "80%")
                var filterComposite = document.createElementNS("http://www.w3.org/2000/svg", 'feComposite')
                filterComposite.setAttribute("in", "SourceGraphic")
                filter.appendChild(filterFlood)
                filter.appendChild(filterComposite)
                defs.appendChild(filter)

                if (d.source.type === "entity" || d.source.type === "relation" || d.source.type === "causeeffect" || /^submodel:(.+)$/.exec(d.source.type) || d.source.type === "attribute" || d.source.type === "probability" || d.source.type === "moderator") {
                    markerArrowL.setAttribute("id", "markerArrowL")
                    markerArrowL.setAttribute("markerWidth", "2")
                    markerArrowL.setAttribute("markerHeight", "4")
                    markerArrowL.setAttribute("refX", "4")
                    markerArrowL.setAttribute("refY", "2")
                    markerArrowL.setAttribute("orient", "180")

                    markerArrowR.setAttribute("id", "markerArrowR")
                    markerArrowR.setAttribute("markerWidth", "2")
                    markerArrowR.setAttribute("markerHeight", "4")
                    markerArrowR.setAttribute("refX", "4")
                    markerArrowR.setAttribute("refY", "2")
                    markerArrowR.setAttribute("orient", "auto")
                    arrowR.setAttribute("d", "M0,0 V4 L2,2 Z")

                    if (d.source.type === "causeeffect" || d.source.type === "probability" || d.source.type === "moderator") {
                        arrowR.setAttribute("style", "fill: black;")
                    } else {
                        arrowR.setAttribute("style", "fill: red;")
                    }
                    arrowL.setAttribute("d", "M0,0 V4 L2,2 Z")
                    arrowL.setAttribute("style", "fill: red;")

                    markerArrowL.appendChild(arrowL)
                    markerArrowR.appendChild(arrowR)
                    defs.appendChild(markerArrowL)
                    defs.appendChild(markerArrowR)
                    pathContents.appendChild(defs)
                ...
                pathLine.setAttribute("class", "red-ui-flow-link-line red-ui-flow-link-path" +
                    (d.link ? " red-ui-flow-link-link" : (activeSubflow ? " red-ui-flow-subflow-link" : "")));
                var id = "text-path-" + d.source.id + ":" + d.target.id
                pathLine.setAttribute("id", id)
                this.__pathLine__ = pathLine;

                pathContents.appendChild(pathLine);

                if ((d.source.type === "entity" && d.target.type === "entity") || (d.source.type === "relation" && d.target.type === "relation") || (d.source.type === "attribute" && d.target.type === "attribute")) {
                    pathLine.setAttribute("marker-start", "url(#markerArrowL")
                    pathText.setAttributeNS("http://www.w3.org/1999/xlink", 'xlink:href', "#" + id)
                    pathText.textContent = "sub"
                    text.setAttribute("dy", "-2.5px")
                    text.appendChild(pathText)
                    pathText.setAttribute("startOffset", "50%")
                    pathContents.appendChild(text)
                    d.target["parent"] = ""
                    d.target["parent"] = d.source.id
                }

                var propertyExists = false
                if ((d.source.type === "relation" && d.target.type === "entity") || (d.source.type === "entity" && d.target.type === "relation")) {
                    if (d.source.type === "relation") {
                        pathLine.setAttribute("marker-end", "url(#markerArrowR)")
                        d.target.props.forEach(prop => {
                            if (prop.v.includes(":")) {
                                let plays = prop.v.split(":")
                                if (plays[0] === d.source.name) {
                                    propertyExists = true
                                    pathText.textContent = plays[1]
                                }
                            }
                        })

                    }
                    if (d.source.type === "entity") {
                        let node = RED.nodes.node(d.source.id)
                        pathLine.setAttribute("marker-start", "url(#markerArrowL)")
                        d.source.props.forEach(prop => {
                            if (prop.v.includes(":")) {
                                let plays = prop.v.split(":")
                                if (plays[0] === d.target.name) {
                                    propertyExists = true
                                    pathText.textContent = plays[1]
                                }
                            }
                    ...
                    text.setAttribute("dy", "-2.5px")
                    pathText.setAttribute("startOffset", "35%")
                    pathContents.appendChild(text)
                }

                // add owns to parent ownership
                if ((d.source.type === "attribute" || d.target.type === "attribute")) {
                    ...
                    if (d.source.type === "relation" || d.source.type === "entity") {
                        let exists = false;
                        d.source.props.forEach((prop) => {
                            // check if attribute exists in current owns list
                            if (prop.p === "owns") {
                                if (d.target.name === prop.v) {
                                    exists = true;
                                    if ((d.target.dbtype !== prop.vt)) {
                                        prop.vt = d.target.dbtype
                                        prop.vtindex = typedb_types[d.target.dbtype]
                                    }
                                }
                            }
                        })
                        if (!exists) {
                            let new_prop = { p: "owns", v: d.target.name, vtindex: typedb_types[d.target.dbtype], vt: d.target.dbtype }
                            d.source.props.push(new_prop)
                        }
                    }

                    if (d.target.type === "relation" || d.target.type === "entity") {
                        let exists = false;
                        d.target.props.forEach((prop) => {
                            // check if attribute exists in current owns list
                            if (prop.p === "owns") {
                                if (d.source.name === prop.v) {
                                    exists = true;
                                    if ((d.source.dbtype !== prop.vt)) {
                                        prop.vt = d.source.dbtype
                                        prop.vtindex = typedb_types[d.source.dbtype]
                                    }
                                }
                            }
                        })
                        if (!exists) {
                            // console.log(" [LINK] Adding target attribute ", d.source.name, " to target ", d.target.name)
                            let new_prop = { p: "owns", v: d.source.name, vtindex: typedb_types[d.source.dbtype], vt: d.source.dbtype }
                            d.target.props.push(new_prop)
                        }
                    }
                }

                // GEMINOS- CAUSAL MODEL
                if ((d.source.type === "causeeffect" || d.source.type === /^submodel:(.+)$/.exec(d.source.type) || d.source.type === "probability" || d.source.type === "moderator") && (d.target.type === "moderator" || d.target.type === "probability" || d.target.type === "causeeffect" || d.target.type === /^submodel:(.+)$/.exec(d.target.type))) {
                    pathText.setAttributeNS("http://www.w3.org/1999/xlink", 'xlink:href', "#" + id);
                    pathText.setAttribute("id", "nrid:" + d.source.id + ":" + d.target.id)
                    pathLine.setAttribute("marker-end", "url(#markerArrowR)")

                    // pathText.setAttribute("filter", "url(#text-rect)")
                    pathText.setAttribute("path", "url(#" + id + ")")
            ...
            link.each(function (d) {
                ...
                if (d.added || d.selected || dirtyNodes[d.source.id] || dirtyNodes[d.target.id]) {
                    ...
                    this.__pathLine__.classList.toggle("red-ui-flow-submodel-link", !d.link && activeSubflow);
                }
            ...
            try {
                pywhyLinkInfo = await RED.causeway.getPyWhyLinkInfo()
                parent_child_indexer = await RED.causeway.getParentChildIndex()
                await RED.causeway.getGPTInfo()
                await RED.causeway.updateTabs()
            } catch (exception) {
                console.log("@view.causeway exception: ", exception)
            }
            // GEMINOS iterate through parent child indexer and find link with given name
            try {
                if (Object.keys(pywhyLinkInfo).length > 0) {
                    let edge_strength = pywhyLinkInfo["edge_strength"]
                    let cms = pywhyLinkInfo["causal_mechanisms"]
                    let add_edge_strength_text = (edge, textPath, type) => {
                        // console.log("edge ", edge, "text path ", textPath, type)
                        // make sure edge is parseFloat type 
                        try {
                            if (type === "degree" || type === "raw") {
                                textPath.textContent = edge
                            }
                            if (type === "percent") {
                                textPath.textContent = edge + "%"
                            }
                        } catch (exception) {
                            // handles stringed format when retrieved from typedb 
                            console.log("exception while adding edge value as textPath ", exception)
                        }
                    }

                    let add_edge_text = async (type = "causal_mechanisms") => {
                        let _fn = "ADD EDGE TEXT"
                        // console.log(`${_fn} Pywhy info ${type} `, pywhyLinkInfo[type])
                        // console.log(`${_fn} strategyType `, RED.causeway.causalAnalysisStrategyType)

                        if (Object.keys(pywhyLinkInfo[type]).length > 0) {
                            for await (let effect of Object.keys(parent_child_indexer)) {
                                effect = RED.nodes.node(effect)
                                try {
                                    effect = effect.__node__
                                } catch (exception) {
                                    console.log("attempted to handle new proxy for effect")
                                }

                                if (type === "edge_strength") {
                                    // calculate the absolute edge strength
                                    let abs_totals = {}
                                    for await (let cause of parent_child_indexer[effect.id]) {
                                        cause = RED.nodes.node(cause)
                                        try {
                                            cause = cause.__node__
                                        } catch (exception) {
                                            console.log("attempted to handle new proxy for cause")
                                        }
                                        if (cause.type === "causeeffect") {
                                            if (edge_strength[effect.id] !== undefined) {
                                                if (edge_strength[effect.id].hasOwnProperty(cause.id)) {
                                                    let add_value = 0;
                                                    if (edge_strength[effect.id][cause.id].hasOwnProperty("median")) {
                                                        add_value = Math.abs(Number(edge_strength[effect.id][cause.id]["median"]))
                                                    }
                                                    else if (typeof edge_strength[effect.id][cause.id] === "string") {
                                                        if ((edge_strength[effect.id][cause.id]).includes("_")) {
                                                            add_value = Math.abs((edge_strength[effect.id][cause.id]).split("_")[0])
                                                        }
                                                        else {
                                                            add_value = Math.abs(Number(edge_strength[effect.id][cause.id]))
                                                        }
                                                    }
                                                    else {
                                                        add_value = Math.abs(Number(edge_strength[effect.id][cause.id]))
                                                    }
                                                    // if effect already has added values continue to add them 
                                                    if (abs_totals[effect.id]) {
                                                        abs_totals[effect.id] += add_value
                                                    } else {
                                                        abs_totals[effect.id] = add_value
                                                    }
                                                }
                                        ...
                                        if (cause.type === "moderator") {
                                            // console.log("getting parent of moderator  ", cause.id)
                                            // console.log("moderator id is part of parent child indexer : ", Object.keys(parent_child_indexer).includes(cause.id))
                                            if (Object.keys(parent_child_indexer).includes(cause.id)) {
                                                if (parent_child_indexer[cause.id].length !== 2) {
                                                    RED.notify(`Moderator node ${cause.id} must have only 2 parents `, "error")
                                                }
                                                if (parent_child_indexer[cause.id].length === 2) {
                                                    // iterate through both causes to get parent name
                                                    // get forward parent moderator 
                                                    let name1 = "mod_" + parent_child_indexer[cause.id].join("_")
                                                    let name2 = "mod_" + parent_child_indexer[cause.id][1] + "_" + parent_child_indexer[cause.id][0]
                                                    // console.log(name1)
                                                    // console.log(name2)
                                                    if (edge_strength[effect.id] !== undefined) {
                                                        let moderator_parent_name = edge_strength[effect.id].hasOwnProperty(name1) ? name1 : name2
                                                        if (edge_strength[effect.id].hasOwnProperty(moderator_parent_name)) {
                                                            // console.log(` edge strength for effect ${effect.name} has moderator parent ${moderator_parent_name}`)
                                                            let add_value = 0;
                                                            if (edge_strength[effect.id][moderator_parent_name].hasOwnProperty("median")) {
                                                                add_value = Math.abs(Number(edge_strength[effect.id][moderator_parent_name]["median"]))
                                                            }
                                                            else if (typeof edge_strength[effect.id][moderator_parent_name] === "string") {
                                                                if ((edge_strength[effect.id][moderator_parent_name]).includes("_")) {
                                                                    add_value = Math.abs((edge_strength[effect.id][moderator_parent_name]).split("_")[0])
                                                                }
                                                                else {
                                                                    add_value = Math.abs(Number(edge_strength[effect.id][moderator_parent_name]))
                                                                }
                                                            }
                                                            else {
                                                                add_value = Math.abs(Number(edge_strength[effect.id][moderator_parent_name]))
                                                            }
                                                            // console.log("ADDING VALUE ", add_value)
                                                            // if effect already has added values continue to add them 
                                                            if (abs_totals[effect.id]) {
                                                                abs_totals[effect.id] += add_value
                                                            } else {
                                    ...
                                    for await (let cause of parent_child_indexer[effect.id]) {
                                        ...
                                        if (cause.type === "causeeffect") {
                                            let path = $("#text-path-" + cause.id + "\\:" + effect.id)
                                            let textPath = d3.select("#nrid\\:" + cause.id + "\\:" + effect.id)
                                            path[0].classList.remove("red-ui-negative-link-line")
                                            path[0].classList.remove("red-ui-positive-link-line")
                                            ...
                                            if (RED.causeway.causalAnalysisStrategyType === "mean") {
                                                ...
                                                //         if (Array.isArray(effect["causalmechanism"])) {
                                                //             let cm = effect["causalmechanism"][0]
                                                //             cm = cm.replace(/'/g, '"');
                                                //             cm = cm.replace(/\(/g, '')
                                                //             cm = cm.replace(/\)/g, '')
                                                //             cm = cm.replace("array", "")
                                                //             cm = cm.replaceAll("True", "true")
                                                //             cm = cm.replaceAll("None", "null")
                                                //             cm = cm.replaceAll("False", "false")
                                                //             // TODO: weird edit when cm is stored as [ " {"prediction_model"} "]
                                                //             if (cm.charAt(0) === '[') {
                                                //                 cm = cm.slice(1, -1);
                                                //                 if (cm.charAt(0) === '"') {
                                                //                     cm = cm.slice(1, -1);
                                                //                 }
                                                //             }
                                                //             effect["causalmechanism"] = JSON.parse(cm)
                                                //         }
                                                //         console.log("effect causalmechanism ", effect["causalmechanism"])
                                                //         if (effect["causalmechanism"].hasOwnProperty("prediction_model")) {
                                                //             if (effect["causalmechanism"]["prediction_model"].hasOwnProperty("parents_during_fit") &&
                                                //                 effect["causalmechanism"]["prediction_model"].hasOwnProperty("coef")
                                                //             ) {
                                                //                 if (effect["causalmechanism"]["prediction_model"].hasOwnProperty("degree")) {
                                                //                     let edge_value = "f(x)^" + effect["causalmechanism"]["prediction_model"]["degree"]
                                                //                     textPath = d3.select("#nrid\\:" + cause.id + "\\:" + effect.id)
                                                //                     textPath = textPath[0][0]
                                                //                     add_edge_strength_text(edge_value, textPath, "degree")

                                                //                     // query edge color 
                                                //                     let edgeColor = await RED.causeway.queryLinkColor(effect)
                                                //                     console.log("EDGE COLOR ", edgeColor)
                                                //                     if (edgeColor !== "grey") {
                                                //                         let pathColor = edgeColor === "red" ? "red-ui-negative-link-line" : "red-ui-positive-link-line"
                                                //                         console.log("PATH COLOR", pathColor)
                                                //                         console.log(path[0])
                                                //                         path[0].classList.add(pathColor)
                                                //                     }

                                                //                 } 
                                                if (cms[effect.id] !== undefined) {
                                                    console.log("\nCAUSAL MECHANISMS ", cms)
                                                    console.log("cause ", cause.name, "effect ", effect.name)
                                                    path = $("#text-path-" + cause.id + "\\:" + effect.id)
                                                    textPath = d3.select("#nrid\\:" + cause.id + "\\:" + effect.id)
                                                    textPath = textPath[0][0]

                                                    console.log(cms[effect.id])
                                                    console.log(cms[effect.id][cause.id])
                                                    let edge_value = cms[effect.id][cause.id]
                                                    console.log("EDGE VALUE", edge_value, typeof edge_value)
                                                    if (edge_value.includes("degree")) {
                                                        let valueParsed = edge_value.replace(/'/g, '"');
                                                        valueParsed = valueParsed.replace(/\(/g, '')
                                                        valueParsed = valueParsed.replace(/\)/g, '')
                                                        valueParsed = valueParsed.replaceAll("array", "")
                                                        valueParsed = valueParsed.replaceAll("True", "true")
                                                        valueParsed = valueParsed.replaceAll("None", "null")
                                                        valueParsed = valueParsed.replaceAll("False", "false")
                                                        valueParsed = JSON.parse(valueParsed)
                                                        edge_value = "f(x)^" + valueParsed["prediction_model"]["degree"]
                                                        add_edge_strength_text(edge_value, textPath, "degree")
                                                        // query edge color 
                                                        let edgeColor = await RED.causeway.queryLinkColor(effect)
                                                        console.log("EDGE COLOR ", edgeColor)
                                                        ...
                                                        let negative = Number(edge_value) < 0 ? true : false
                                                        edge_value = (typeof edge_value === "string") ? Number.parseFloat(edge_value) : edge_value
                                                        edge_value = edge_value.toLocaleString(undefined, { maximumFractionDigits: 2, minimumFractionDigits: 2 });
                                                        if (edge_value !== "-0" && edge_value !== "-0.00") {
                                                            if (negative) {
                                                                path[0].classList.add("red-ui-negative-link-line")
                                                            } else {
                                                                path[0].classList.add("red-ui-positive-link-line")
                                                            }
                                                        } else {
                                                            edge_value = "0.00"
                                                        }
                                                        add_edge_strength_text(edge_value, textPath, "raw")

                                                    }
                                                }
                                            }
                                            if (RED.causeway.causalAnalysisStrategyType === "variance") {
                                                if (edge_strength[effect.id] !== undefined) {
                                                    if (edge_strength[effect.id].hasOwnProperty(cause.id)) {
                                                        console.log(edge_strength[effect.id][cause.id])
                                                        let edge_value = 0;
                                                        if (edge_strength[effect.id][cause.id].hasOwnProperty("median")) {
                                                            edge_value = edge_strength[effect.id][cause.id]["median"]
                                                        }
                                                        else if (typeof edge_strength[effect.id][cause.id] === "string") {
                                                            if ((edge_strength[effect.id][cause.id]).includes("_")) {
                                                                edge_value = (edge_strength[effect.id][cause.id]).split("_")[0]
                                                            } else {
                                                                edge_value = Number(edge_strength[effect.id][cause.id])
                                                            }
                                                        }
                                                        else {
                                                            edge_value = edge_strength[effect.id][cause.id]
                                                        }
                                                        let edge_percent = Math.abs(Number(edge_value))
                                                        let calculation_type = "abs"
                                                        if (calculation_type === "abs") {
                                                            edge_percent = (Math.abs(Number(edge_value)) / abs_totals[effect.id]) * 100
                                                        }
                                                        path = $("#text-path-" + cause.id + "\\:" + effect.id)
                                                        textPath = d3.select("#nrid\\:" + cause.id + "\\:" + effect.id)
                                                        edge_value = (typeof edge_value === "string") ? Number.parseFloat(edge_value) : edge_value
                                                        edge_value = edge_value.toLocaleString(undefined, { maximumFractionDigits: 2, minimumFractionDigits: 2 });
                                                        edge_percent = edge_percent.toLocaleString(undefined, { maximumFractionDigits: 2, minimumFractionDigits: 2 });
                                                        let negative = Number(edge_value) < 0 ? true : false
                                                        textPath = textPath[0][0]
                                                        // shorten the edge percent
                                                        if (effect.showedgeval) {
                                                            if (calculation_type === "abs") {
                                                                add_edge_strength_text(edge_percent, textPath, "percent")
                                                            } else {
                                                                add_edge_strength_text(edge_percent, textPath, "raw")
                                                            }
                                                            if (negative) {
                                                                path[0].classList.add("red-ui-negative-link-line")
                                                            } else {
                                                        ...
                                                        } else if (effect.showicalingam) {
                                                            if (effect.hasOwnProperty("icalingam")) {
                                                                if (effect["icalingam"].hasOwnProperty(cause.name)) {
                                                                    console.log("EFFECT ICALINGAM ", effect["icalingam"][cause.name])
                                                                }
                                                            }
                                                        }
                                                    }
                                        ...
                                        if (cause.type === "moderator") {
                                            ...
                                            if (RED.causeway.causalAnalysisStrategyType === "mean") {
                                                ...
                                                //                 if (effect["causalmechanism"]["prediction_model"].hasOwnProperty("degree")) {
                                                //                     let iter = 0
                                                //                     for await (let interaction of effect["causalmechanism"]["prediction_model"]["interactions"]) {
                                                //                         if (interaction[0] === 1 && interaction[1] === 1) {
                                                //                             console.log("interaction identified multiplier", iter)
                                                //                             // then that means the coefficient of this iter value can go on the edge path 
                                                //                             let edge_value = effect["causalmechanism"]["prediction_model"]["coef"][iter]
                                                //                             textPath = d3.select("#nrid\\:" + cause.id + "\\:" + effect.id)
                                                //                             textPath = textPath[0][0]
                                                //                             let negative = Number(edge_value) < 0 ? true : false
                                                //                             console.log(" causal path : ", cause.id, " ", effect.id, " ", edge_value)
                                                //                             edge_value = (typeof edge_value === "string") ? Number.parseFloat(edge_value) : edge_value
                                                //                             edge_value = edge_value.toLocaleString(undefined, { maximumFractionDigits: 2, minimumFractionDigits: 2 });
                                                //                             console.log("EDGE VALUE ", edge_value)
                                                ...
                                                //                 } else {
                                                //                     if (cause.type === "moderator") {
                                                //                         //get parents of cause
                                                //                         let parents = parent_child_indexer[cause.id]
                                                //                         let name1 = "mod_" + parents.join("_")
                                                //                         let name2 = "mod_" + parents[1] + parents[0]
                                                //                         if (effect["causalmechanism"]["prediction_model"]["parents_during_fit"].includes(name1)) {
                                                //                             cause.name = name1
                                                //                         } else {
                                                //                             cause.name = name2
                                                //                         }
                                                //                     }
                                                //                     let causeIndex = effect["causalmechanism"]["prediction_model"]["parents_during_fit"].indexOf(cause.name)
                                                //                     let edge_value = effect["causalmechanism"]["prediction_model"]["coef"][causeIndex]
                                                //                     console.log("moderator causal path : ", cause.id, " ", effect.id, " ", edge_value)
                                                //                     path = $("#text-path-" + cause.id + "\\:" + effect.id)
                                                //                     textPath = d3.select("#nrid\\:" + cause.id + "\\:" + effect.id)
                                                //                     edge_value = (typeof edge_value === "string") ? Number.parseFloat(edge_value) : edge_value
                                                //                     edge_value = edge_value.toLocaleString(undefined, { maximumFractionDigits: 2, minimumFractionDigits: 2 });
                                                ...
                                                // }
                                                if (cms[effect.id] !== undefined) {
                                                    // console.log("MODERATOR CAUSAL MECHANISMS ", cms)
                                            ...
                                            if (RED.causeway.causalAnalysisStrategyType === "variance") {
                                                if (edge_strength[effect.id] !== undefined) {
                                                    if (edge_strength[effect.id].hasOwnProperty(cause.name)) {
                                                        let edge_value = 0;
                                                        if (edge_strength[effect.id][cause.name].hasOwnProperty("median")) {
                                                            edge_value = edge_strength[effect.id][cause.name]["median"]
                                                        }
                                                        else if (typeof edge_strength[effect.id][cause.name] === "string") {
                                                            if ((edge_strength[effect.id][cause.name]).includes("_")) {
                                                                edge_value = (edge_strength[effect.id][cause.name]).split("_")[0]
                                                            } else {
                                                                edge_value = Number(edge_strength[effect.id][cause.name])
                                                            }
                                                        }
                                                        else {
                                                            edge_value = edge_strength[effect.id][cause.name]
                                                        }
                                                        let edge_percent = Math.abs(Number(edge_value))
                                                        let calculation_type = "abs"
                                                        if (calculation_type === "abs") {
                                ...
                                // handles mean
                                if (type === "causal_mechanisms") {
                                    let cms = pywhyLinkInfo[type]
                                    for await (let effect of Object.keys(parent_child_indexer)) {
                                        effect = RED.nodes.node(effect)
                                        try {
                                            effect = effect.__node__
                                        ...
                                        for await (let cause of parent_child_indexer[effect.id]) {
                                            ...
                                            if (cause.type === "causeeffect") {
                                                if (cms[effect.id] !== undefined) {
                                                    ...
                                                    let parents = []
                                                    let cm = {}
                                                    // CM is of type object when we execute a fresh instance of core analysis
                                                    if (typeof cms[effect.id] === "object") {
                                                        if (cms[effect.id].hasOwnProperty("0")) {
                                                            if (typeof cms[effect.id][0] === "string") {
                                                                cm = cms[effect.id][0].replace(/'/g, '"');
                                                                cm = cm.replace(/\(/g, '')
                                                                cm = cm.replace(/\)/g, '')
                                                                cm = cm.replace("array", "")
                                                                cm = cm.replaceAll("True", "true")
                                                            ...
                                                            if (cm.hasOwnProperty("prediction_model")) {
                                                                if (cm["prediction_model"].hasOwnProperty("coef")) {
                                                                    coefs = cm["prediction_model"]["coef"]
                                                                }
                                                                if (cm["prediction_model"].hasOwnProperty("parents_during_fit")) {
                                                                    parents = cm["prediction_model"]["parents_during_fit"]
                                                                }
                                                            }
                                                        }
                                                    }
                                                    // iterate through the parents to match the corresponding value
                                                    parents.forEach(async (parent, index) => {
                                                        if (parent === cause.name) {
                                                            // console.log("parent matche cause.name")
                                                            let path = $("#text-path-" + cause.id + "\\:" + effect.id)
                                                            let textPath = d3.select("#nrid\\:" + cause.id + "\\:" + effect.id)
                                                            textPath = textPath[0][0]
                                                            let edge_value = coefs[index]
                                                            let negative = Number(edge_value) < 0 ? true : false
                                                            // console.log(edge_value, negative)
                                                            console.log("EFFE SHOW EDGE VALUE ", effect.showedgeval)
                                                            if (effect.showedgeval) {
                                                                add_edge_strength_text(edge_value, textPath, "raw")
                                                                if (negative) {
                                                                    path[0].classList.add("red-ui-negative-link-line")
                    ...
                    // CAUSAL MECHANISM REGRESSION ON EDGE
                    // add_edge_text(type = "causal_mechanisms")
                    add_edge_text(type = "edge_strength")
                }
            } catch (exception) {
    ...
    function importNodes(newNodesObj, options) {
        ...
        try {
            ...
            var activeSubmodelChanged;
            if (activeSubflow) {
                activeSubflowChanged = activeSubflow.changed;
            }

            if (activeSubmodel) {
                activeSubmodelChanged = activeSubmodel.changed;
            }
            ...
            if (result) {
                ...
                var new_submodels = result.submodels;
    ...
    function createNode(type, x, y, z) {
        ...
        var c = /^submodel:(.+)$/.exec(type);

        var activeSubflow = z ? RED.nodes.subflow(z) : null;
        var activeSubmodel = z ? RED.nodes.submodel(z) : null;
        ...
        } else if (activeSubmodel && c) {
            var submodelId = c[1];
            if (submodelId === activeSubmodel.id) {
                throw new Error(RED._("notification.error", { message: RED._("notification.errors.cannotAddSubmodelToItself") }))
            }
            if (RED.nodes.submodelContains(c[1], activeSubmodel.id)) {
                throw new Error(RED._("notification.error", { message: RED._("notification.errors.cannotAddCircularReference") }))
            }
        }
        ...
        if (!m) {
            ...
            var submodel = RED.nodes.submodel(c[1])
            nn.name = " ";
            nn.inputs = submodel.in.length;
            nn.outputs = submodel.out.length;
        }
        ...
        if (activeSubmodel) {
            var submodelRefresh = RED.subflow.refresh(true);
            if (submodelRefresh) {
                historyEvent.submodel = {
                    id: activeSubmodel.id,
                    changed: activeSubmodel.changed,
    ...
    function calculateNodeDimensions(node) {
        ...
        try {
            var isLink = (node.type === "link in" || node.type === "link out" || node.type === "probability" || node.type === "moderator")
    ...
    return {
        ...
        reveal: function (id, triggerHighlight) {
            try {
                console.log("AT REVEAL RED.view.reveal", id, triggerHighlight)
                ...
                console.log("reveal exception ", exception)
            }
        },
        gridSize: function (v) {
            if (v === undefined) {

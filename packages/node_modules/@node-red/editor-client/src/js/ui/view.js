...
RED.view = (function () {
    ...
    async function redraw_causeffect_edge(path, edge_value) {
                ...
                if (Object.keys(pywhyLinkInfo).length > 0) {
                    ...
                    let add_edge_strength_text = (edge, textPath, type) => {
                        ...
                        try {
                            ...
                            if (type === "percent") {
                                // console.log("updating text path with %")
                                textPath.textContent = edge + "%"
                            }
                        } catch (exception) {
                                    ...
                                    if (cause.type === "causeeffect") {
                                        if (edge_strength[effect.id] !== undefined) {
                                            if (edge_strength[effect.id].hasOwnProperty(cause.id)) {
                                                ...
                                                } else if (typeof edge_strength[effect.id][cause.id] === "object") {
                                                    if (typeof edge_strength[effect.id][cause.id].hasOwnProperty("causal_strength_type")) {
                                                        add_value = Math.abs(Number(edge_strength[effect.id][cause.id]["causal_strength_value"]))
                                                    }
                                                }
                                                else {
                                                    add_value = Math.abs(Number(edge_strength[effect.id][cause.id]))
                                ...
                                if (cause.type === "causeeffect") {
                                    if (RED.causeway.causalAnalysisStrategyType === "mean") {
                                        if (cms[effect.id] !== undefined) {
                                            // console.log(`${cause.name} ====>>> ${effect.name} `, cms[effect.id])

                                            // console.log(" PATH class list ", path[0].classList)
                                            if (cms[effect.id].hasOwnProperty(cause.id)) {
                                                let edge_object = cms[effect.id][cause.id]
                                                // console.log(`${cause.id} ====>>> ${effect.id} `,edge_object, edge_object.length)
                                                if (typeof edge_value === "string") {
                                                ...
                                                // new implementation includes an object
                                                let _edge_text = ""

                                                // case where we have multiterm polynomial such as mx^2+bx+c
                                                if (edge_object.length > 1) {
                                                    // look for highest degree
                                                    let degree = 0
                                                    for await (let cs of edge_object) {
                                                        if (cs.hasOwnProperty("causal_strength_degree")) {
                                                            if (cs["causal_strength_degree"] > degree) {
                                                                degree = cs["causal_strength_degree"]
                                                            }
                                                        }
                                                    }
                                                    _edge_text = "f(x)^" + degree

                                                } else {
                                                    // case where we have a single high power term  mx^2 + c
                                                    for await (let cs of edge_object) {
                                                        // console.log(`cs object `, cs)
                                                        if (cs["causal_strength_degree"] < 2) {
                                                            let edge_value = cs["causal_strength_value"]
                                                            if (edge_value === null) {
                                                                edge_value = 0
                                                            }
                                                            ...
                                                            if (edge_value !== "-0" && edge_value !== "-0.00") {
                                                                if (negative) {
                                                                    // console.log("path was previously NEGATIVE ", path[0].classList.contains("red-ui-negative-link-line"))
                                                                    ...
                                                                    // console.log("path was previously POSITIVE ", path[0].classList.contains("red-ui-positive-link-line"))
                                                            ...
                                                            _edge_text = edge_value
                                                            // add_edge_strength_text(_edge_text, textPath, "raw")

                                                        } else {
                                                            _edge_text = "f(x)^" + cs["causal_strength_degree"]
                                    ...
                                    if (RED.causeway.causalAnalysisStrategyType === "variance") {
                                        // console.log("RED.VIEW VARIANCE", edge_strength, effect.id, edge_strength.hasOwnProperty(effect.id))
                                        if (edge_strength.hasOwnProperty(effect.id)) {
                                            // console.log("edge strength effect : ", edge_strength[effect.id])
                                            if (edge_strength[effect.id] !== undefined) {
                                                if (edge_strength[effect.id].hasOwnProperty(cause.id)) {
                                                    // console.log("edge strength cause -> effect: ", edge_strength[effect.id][cause.id])
                                                    let edge_value = 0;
                                                    if (edge_strength[effect.id][cause.id].hasOwnProperty("median")) {
                                                        ...
                                                        if (edge_strength[effect.id][cause.id].hasOwnProperty("causal_strength_type")) {
                                                            edge_value = edge_strength[effect.id][cause.id]["causal_strength_value"]
                                                        }
                                                    }
                                                    else {
                                                    ...
                                                    if (calculation_type === "abs") {
                                                        // console.log("number edge value: ", Number(edge_value))
                                                        // console.log("effect ", effect.id)
                                                        // console.log(effect)
                                                        // console.log("absolute total : ", abs_totals[effect.id])
                                                        if (abs_totals[effect.id] === 0 || abs_totals[effect.id] === 0.0) {
                                                            edge_percent = 100
                                                        } else {
                                ...
                                if (cause.type === "moderator") {
                                    if (RED.causeway.causalAnalysisStrategyType === "mean") {
                                        if (cms[effect.id] !== undefined) {
                                            ...
                                            textPath.textContent = edge_value
                                    ...
                                    if (RED.causeway.causalAnalysisStrategyType === "variance") {
                                        console.log("VARIANCE HERE ", edge_strength[effect.id])
                                        if (edge_strength[effect.id] !== undefined) {
                                            if (edge_strength[effect.id].hasOwnProperty(cause.id)) {
                                                let edge_value = 0;
                                                if (edge_strength[effect.id][cause.id].hasOwnProperty("median")) {
                                                ...
                                                if (effect.showedgeval) {
                                                    ...
                                                    if (effect.hasOwnProperty("icalingam")) {
                                                        if (effect["icalingam"].hasOwnProperty(cause.id)) {
                                                            console.log("EFFECT ICALINGAM ", effect["icalingam"][cause.id])
                                                        }
                                                    }
                                                }
                                            }

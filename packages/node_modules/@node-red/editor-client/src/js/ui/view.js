...
RED.view = (function () {
    ...
    async function _redraw() {
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            console.log("RED.causeway:", RED.causeway)
            ...
            try {
                console.log("pyhwhy info", pywhyinfo, typeof pywhyinfo)
                if (Object.keys(pywhyinfo).length > 0) {
                    ...
                    let add_edge_strength_text = (edge, textPath, type) => {
                        ...
                        try {
                            ...
                            if (type === "raw") {
                                textPath.textContent = edge.toPrecision(5)
                            }
                            if (type === "percent") {
                                if (edge > 0 && edge < 1 || edge < 0 && edge > -1) {
                                    textPath.textContent = edge
                                    textPath.textContent = edge.toPrecision(2) + "%"
                                } else {
                                    textPath.textContent = edge.toPrecision(3) + "%"
                                }
                            }
                        } catch (exception) {
                                        ...
                                        if (cause.type === "causeeffect") {
                                            if (edge_strength[effect.id] !== undefined) {
                                                if (edge_strength[effect.id].hasOwnProperty(cause.name)) {
                                                    ...
                                                    let edge_percent = (Math.abs(Number(edge_value)) / abs_totals[effect.id]) * 100
                    ...
                    // check if any causal me
                    add_edge_text(type = "causal_mechanisms")

                    /*
                    if edge strengths dictionary containg parent-child info exists 
                    then map the node-red link to the parent-child edge strength 
                    */
                    // if (Object.keys(edge_strength).length > 0) {
                    //     for await (let effect of Object.keys(parent_child_indexer)) {
                    //         effect = RED.nodes.node(effect)
                    //         try {
                    //             effect = effect.__node__
                    ...
                    //             console.log("attempted to handle new proxy for effect")
                    //         }
                    //         // console.log("EFFECT", effect.name, effect.id)
                    //         let abs_totals = {}
                    //         for await (let cause of parent_child_indexer[effect.id]) {
                    //             cause = RED.nodes.node(cause)
                    //             try {
                    ...
                    //                 }
                    //             }
                    //             if (cause.type === "probability") {
                    //                 // get parents of probability
                    //                 for await (let gcause of parent_child_indexer[cause.id]) {
                    //                     gcause = RED.nodes.node(gcause)
                    //                     if (gcause.type === "causeeffect") {
                    //                         let textPath = d3.select("#nrid\\:" + gcause.id + "\\:" + cause.id)
                    //                         textPath = textPath[0][0]
                    //                         // add_edge_strength_text(edge_strength[effect.id][gcause.name], textPath)
                    //                     }
                    //                 }
                    //             }
                    //         }
                    ...
                    //                         let path = $("#text-path-" + cause.id + "\\:" + effect.id)
                    //                         if (effect.showeffects) {
                    //                             add_edge_strength_text(edge_percent, textPath)
                    //                             if (negative) {
                    //                                 path[0].classList.add("red-ui-positive-link-line")
                    //                             } else {
                    //                                 path[0].classList.add("red-ui-negative-link-line")

                ...
                let add_edge_strength_text = (edge, textPath) => {
                    try {
                        ...
                        if (edge > 0 && edge < 1 || edge > 0 && edge > -1) {
                            textPath.textContent = edge.toFixed(3) === "0.000" ? "0" : edge.toFixed(3)
                        } else {
                            textPath.textContent = Math.floor(edge)
                        }
                    } catch (exception) {
                        // handles stringed format when retrieved from typedb 
                        if (edge > 0 && edge < 1 || edge > 0 && edge > -1) {
                            textPath.textContent = parseFloat(edge).toFixed(3) === "0.000" ? "0" : parseFloat(edge).toFixed(3)
                        } else {
                            textPath.textContent = Math.floor(parseFloat(edge))
                        }
                    }
                }
                        ...
                        for await (let cause of parent_child_indexer[effect.id]) {
                            ...
                            if (cause.type === "causeeffect") {
                                if (edge_strength[effect.id] !== undefined) {
                                    ...
                                    add_edge_strength_text(edge_strength[effect.id][cause.name], textPath)
                                }
                            }

            ...
            try {
                ...
                if (Object.keys(pywhy_info).length > 0) {
                    ...
                    let add_edge_strength_text = (edge, textPath) => {
                        console.log("ADDING EDGE STRENGTH")
                            ...
                            for await (let cause of parent_child_indexer[effect.id]) {
                                ...
                                if (cause.type === "causeeffect") {
                                    if (edge_strength[effect.id] !== undefined) {
                                        console.log("edge strenght: ", Number(edge_strength[effect.id][cause.name]))
                                        if (abs_totals[effect.id]) {
                                            abs_totals[effect.id] += Math.abs(Number(edge_strength[effect.id][cause.name]))
                                        } else {
                                            abs_totals[effect.id] = Math.abs(Number(edge_strength[effect.id][cause.name]))
                                        }
                                    }
                            ...
                            for await (let cause of parent_child_indexer[effect.id]) {
                                ...
                                if (cause.type === "causeeffect") {
                                    if (edge_strength[effect.id] !== undefined) {
                                        ...
                                        let edge_percent = Math.abs(Number(edge_strength[effect.id][cause.name])) / abs_totals[effect.id]
                                        let textPath = d3.select("#nrid\\:" + cause.id + "\\:" + effect.id)
                                        let negative = Number(edge_strength[effect.id][cause.name]) < 0 ? true : false
                                        textPath = textPath[0][0]
                                        console.log("!!!!!\ntextPath: ", textPath)
                                        console.log(effect)
                                        ...
                                        if (effect.showeffects) {
                                            ...
                                            console.log(path[0])
                                            if (negative) {
                                                console.log("class list: ", path[0].classList)

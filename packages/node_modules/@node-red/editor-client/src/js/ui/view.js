                                    ...
                                    for await (let cause of parent_child_indexer[effect.id]) {
                                        ...
                                        if (cause.type === "moderator") {
                                            ...
                                            if (Object.keys(parent_child_indexer).includes(cause.id)) {
                                                console.log("get parent of moderator ")
                                                console.log("NOTE should only be one parent")
                                                ...
                                                if (parent_child_indexer[cause.id].length === 2) {
                                                    ...
                                                    for await (let moderator_parent_id of parent_child_indexer[cause.id]) {
                                                        console.log("moderate parent: ", moderator_parent_id)
                                                        let moderator_parent = RED.nodes.node(moderator_parent_id)
                                                        console.log("moderator parent: ", moderator_parent.name, moderator_parent)
                                                        if (edge_strength[effect.id] !== undefined) {
                                                            if (edge_strength[effect.id].hasOwnProperty(moderator_parent.name)) {
                                                                console.log(` edge strength for effect ${effect.name} has moderator parent ${moderator_parent.name}`)
                                                                let add_value = 0;
                                                                if (edge_strength[effect.id][moderator_parent.name].hasOwnProperty("median")) {
                                                                    add_value = Math.abs(Number(edge_strength[effect.id][moderator_parent.name]["median"]))
                                                                }
                                                                else if (typeof edge_strength[effect.id][moderator_parent.name] === "string") {
                                                                    if ((edge_strength[effect.id][moderator_parent.name]).includes("_")) {
                                                                        add_value = Math.abs((edge_strength[effect.id][moderator_parent.name]).split("_")[0])
                                                                    }
                                                                    else {
                                                                        add_value = Math.abs(Number(edge_strength[effect.id][moderator_parent.name]))
                                                                    }
                                                                }
                                                                else {
                                                                    add_value = Math.abs(Number(edge_strength[effect.id][moderator_parent.name]))

...
RED.view = (function () {
    ...
    function init() {
        ...
        chart.droppable({
            ...
            drop: function (event, ui) {
                ...
                try {
                    var isLink = (nn.type === "link in" || nn.type === "link out" || nn.type === "probability")
                    var hideLabel = nn.hasOwnProperty('l') ? !nn.l : isLink;
                    console.log(nn.type)
                    if (nn.type === "probability") { hideLabel = true }
    ...
    async function _redraw() {
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            linkEnter.each(async function (d, i) {
                ...
                if (d.source.type === "entity" || d.source.type === "relation" || d.source.type === "causeeffect" || /^submodel:(.+)$/.exec(d.source.type) || d.source.type === "attribute" || d.source.type === "probability") {
                    ...
                    if (d.source.type === "causeeffect" || d.source.type === "probability") {
                ...
                if ((d.source.type === "causeeffect" || d.source.type === /^submodel:(.+)$/.exec(d.source.type) || d.source.type === "probability") && (d.target.type === "probability" || d.target.type === "causeeffect" || d.target.type === /^submodel:(.+)$/.exec(d.target.type))) {
                    pathText.setAttributeNS("http://www.w3.org/1999/xlink", 'xlink:href', "#" + id);
                            ...
                            for await (let effect of Object.keys(parent_child_indexer)) {
                                ...
                                if (type === "edge_strength") {
                                    ...
                                    for await (let cause of parent_child_indexer[effect.id]) {
                                        ...
                                        // console.log("CAUSE:", cause.name, cause.id)
                                        ...
                                        if (cause.type === "causeeffect") {
                                            ...
                                            if (RED.causeway.causalAnalysisStrategyType === "mean") {
                                                if (effect.hasOwnProperty("causalmechanism")) {
                                                    ...
                                                    // console.log(effect)
                                                    if (effect["causalmechanism"].hasOwnProperty("causes")) {
                                                        if (effect["causalmechanism"]["prediction_model"].hasOwnProperty("parents_during_fit") &&
                                                            ...
                                                            if (effect["causalmechanism"]["prediction_model"].hasOwnProperty("degree")) {
                                                                ...
                                                                // console.log(`${cause.name} => ${effect.name} `, effect["causalmechanism"]["prediction_model"]["coef"][causeIndex])
                                            ...
                                            if (RED.causeway.causalAnalysisStrategyType === "variance") {
                                                console.log("causalAnalysisStrategyType is of type variance ")
                                                console.log("edge strength is not undefined " ,edge_strength[effect.id] !== undefined)
                                                console.log("cause name : ", cause.name)
                                                console.log(edge_strength[effect.id])
                                                if (edge_strength[effect.id] !== undefined) {
                                                    console.log("edge has cause : " , edge_strength[effect.id].hasOwnProperty(cause.name))
                                                    if (edge_strength[effect.id].hasOwnProperty(cause.name)) {
                                                        console.log("HERE")
                                                        let edge_value = 0;
                                                        if (edge_strength[effect.id][cause.name].hasOwnProperty("median")) {
    ...
    function calculateNodeDimensions(node) {
        ...
        try {
            var isLink = (node.type === "link in" || node.type === "link out" || node.type === "probability")

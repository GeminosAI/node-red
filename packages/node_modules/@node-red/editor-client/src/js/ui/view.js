...
RED.view = (function () {
    ...
    function deleteSelection(reconnectWires) {
        ...
        if (workspaceSelection.length > 0) {
            ...
            // geminos
            if (submodelInstances) {
                historyEvent.submodelOutputs = removedSubmodelOutputs,
                    historyEvent.submodelInputs = removedSubmodelInputs,
                    historyEvent.submodel = {
                        id: activeSubmodel ? activeSubmodel.id : undefined,
                        instances: submodelInstances
                    }
            }
            if (removedSubmodelStatus) {
                historyEvent.submodel.status = removedSubmodelStatus;
            }
            // end of geminos
    ...
    function _redraw() {
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            let result = RED.causeway.getBackdoors()
            ...
            linkEnter.each(function (d, i) {
                ...
                if ((d.source.type === "causeeffect" || /^submodel:(.+)$/.exec(d.source.type) || "probability") && (d.target.type === "causeeffect" || /^submodel:(.+)$/.exec(d.target.type))) {
                ...
                if ((d.source.type === "causeeffect" || /^submodel:(.+)$/.exec(d.source.type) || "probability") && (d.target.type === "probability")) {
                    pathText.setAttributeNS("http://www.w3.org/1999/xlink", 'xlink:href', "#" + id);
                    // link should tell us the cause node effect and show on the path
                    pathText.textContent = Math.random().toFixed(4).toString()
                    ...
                    text.appendChild(pathText)
                    pathText.setAttribute("startOffset", "20%")
                    pathContents.appendChild(text)
                }
                if ((d.source.type === "causeeffect" || /^submodel:(.+)$/.exec(d.source.type)) && (d.target.type === "causeeffect" || /^submodel:(.+)$/.exec(d.target.type))) {
                    pathText.setAttributeNS("http://www.w3.org/1999/xlink", 'xlink:href', "#" + id);
                    // link should tell us the cause node effect and show on the path
                    pathText.textContent = Math.random().toFixed(4).toString()
                    pathText.setAttribute("filter", "url(#text-rect)")

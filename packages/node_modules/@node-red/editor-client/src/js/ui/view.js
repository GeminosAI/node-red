        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            linkEnter.each(async function (d, i) {
                ...
                if ((d.source.type === "causeeffect" || /^submodel:(.+)$/.exec(d.source.type) || "probability") && (d.target.type === "probability" || "causeeffect" || /^submodel:(.+)$/.exec(d.target.type))) {
                    ...
                    // }
                    pathText.setAttribute("id", d.source.id + ":" + d.target.id)
            ...
            if (Object.keys(pywhy_info).length > 0) {
                ...
                if (Object.keys(edge_strength).length > 0) {
                    d3.selectAll("textPath").forEach(async (textPaths) => {
                        for await (let textPath of textPaths) {
                            let edge_id = textPath.id.split(":")
                            let parent_n = edge_id[0]
                            let child_n = edge_id[1]
                            parent_n = RED.nodes.node(parent_n)
                            child_n = RED.nodes.node(child_n)
                            let parent_names = []
                            for await (let prop of parent_n.props) {
                                if (prop.p === "input") {
                                    parent_names.push(prop.attribute)
                                }
                            }
                            let child_names = []
                            for await (let prop of child_n.props) {
                                if (prop.p === "input") {
                                    child_names.push(prop.attribute)
                                }
                            }

                            for await (let child_name of child_names) {
                                for await (let parent_name of parent_names) {
                                    if (edge_strength[child_name][parent_name]) {
                                        try {
                                            textPath.textContent = edge_strength[child_name][parent_name].toFixed(4)
                                        } catch (exception) {
                                            textPath.textContent = parseFloat(edge_strength[child_name][parent_name]).toFixed(4)
                                        }
                                    } else {
                    ...
                    }) //end of d3 select all text path loop
                }
            }

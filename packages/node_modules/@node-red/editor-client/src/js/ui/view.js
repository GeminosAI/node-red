    ...
    async function _redraw() {
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            nodeEnter.each(function (d, i) {
                ...
                if (d.treatment) {
                    // console.log("NODE IS OF TYPE TREATMENT", d.name)
                    mainRect.setAttribute("style", "stroke:#32CD32;stroke-width:3; opacity:0.9;")
                }
                if (d.outcome) {
                    // console.log("NODE IS OF TYPE TREATMENT", d.name)
                    mainRect.setAttribute("style", "stroke:yellow;stroke-width:3; opacity:0.9;")
                }
            ...
            linkEnter.each(async function (d, i) {
                ...
                if (d.source.type === "entity" || d.source.type === "relation" || d.source.type === "causeeffect" || /^submodel:(.+)$/.exec(d.source.type) || d.source.type === "attribute" || d.source.type === "probability") {
                    ...
                    if (d.source.type === "causeeffect" || "probability") {
                ...
                if ((d.source.type === "entity" && d.target.type === "relation") || (d.source.type === "relation" && d.target.type === "entity")) {
                ...
                if ((d.source.type === "attribute" || d.target.type === "attribute")) {
                    // console.log(" [LINK] D-Source ", d.source)
                    // console.log(" [LINK] D-Target ", d.target)
                    // check all the properties of the source if it is a relation // entity
                    // console.log(d)
                    ...
                    if (d.source.type === "relation" || d.source.type === "entity") {
                            ...
                            if (prop.p === "owns") {
                                if (d.target.name === prop.v) {
                                    ...
                                    if ((d.target.dbtype !== prop.vt)) {
                                        console.log("Updating " + prop.vt + " to " + d.target.dbtype)
                        ...
                        if (!exists) {
                            // console.log(" [LINK] Adding target attribute ", d.target.name, " to source ", d.source.name)
                            let new_prop = { p: "owns", v: d.target.name, vtindex: typedb_types[d.target.dbtype], vt: d.target.dbtype }
                            d.source.props.push(new_prop)
                            // console.log("adding attribute: ",new_prop)
                        } else {
                            // console.log( " [LINK] target attribute ", d.target.name, " already exists in " , d.source.name)
                        }
                    }
                    ...
                    if (d.target.type === "relation" || d.target.type === "entity") {
                        ...
                        d.target.props.forEach((prop) => {
                            ...
                            if (prop.p === "owns") {
                                if (d.source.name === prop.v) {
                                    ...
                                    // console.log(d.source.dbtype)
                                    // console.log(prop.vt)
                                    if ((d.source.dbtype !== prop.vt)) {
                                        // console.log("Updating " + prop.vt + " to "+ d.source.dbtype)
                        ...
                        if (!exists) {
                            ...
                            // console.log( " [LINK] target attribute ", d.source.name, " already exists in " , d.target.name)
                        }
                    }
                }
                ...
                if ((d.source.type === "causeeffect" || /^submodel:(.+)$/.exec(d.source.type) || "probability") && (d.target.type === "probability" || "causeeffect" || /^submodel:(.+)$/.exec(d.target.type))) {
                    pathText.setAttributeNS("http://www.w3.org/1999/xlink", 'xlink:href', "#" + id);

                    // if (d.target.type === "causeeffect") {
                    //     // find edge in

                    //     let parent_child_indexer = await RED.causeway.getParentChildIndex()
                    //     // probability nodes
                    //     for await (let parent of parent_child_indexer[d.target.id]) {
                    //         // causeeffects connected to probability nodes
                    //         for await (let causeeffect of parent_child_indexer[parent]) {
                    //             causeeffect = RED.nodes.node(causeeffect)
                    //             console.log("causeeffect: ", causeeffect)
                    //         }
                    //     }
                    ...
                    text.appendChild(pathText)
                    pathText.setAttribute("startOffset", "25%")
                    pathContents.appendChild(text)
                }
            ...
            let pywhy_info = await RED.causeway.getPyWhyLinkInfo()
            console.log("pywhy info : ", pywhy_info)
            let parent_child_indexer = await RED.causeway.getParentChildIndex()
            if (Object.keys(pywhy_info).length > 0) {
                let edge_strength = pywhy_info["pywhy-gcm"]["edge_strength"]
                console.log(edge_strength)
                if (Object.keys(edge_strength).length > 0) {
                    console.log("parent child indexer: ", parent_child_indexer)
                        ...
                        for await (let cause of parent_child_indexer[effect.id]) {
                            ...
                            if (cause.type === "causeeffect") {
                                if (edge_strength[effect.id] !== undefined) {
                                    ...
                                    console.log("edge strength of probability node: ", edge_strength[effect.id])
                                    let textPath = d3.select("#nrid\\:" + cause.id + "\\:" + effect.id)
                                    textPath = textPath[0][0]

                                    try {
                                        // handles recent returend values of type double from pywhy causal inf implementation 
                                        textPath.textContent = edge_strength[effect.id][cause.name].toFixed(4)
                                    } catch (exception) {
                                        // handles stringed format when retrieved from typedb 
                                        textPath.textContent = parseFloat(edge_strength[effect.id][cause.name]).toFixed(4)
                                    }
                                }
                            }
                            if (cause.type === "probability") {
                                ...
                                for await (let gcause of parent_child_indexer[cause.id]) {
                                    ...
                                    if (gcause.type === "causeeffect") {
                                        console.log("effect ", effect)
                                        console.log("causes ", gcause)
                                        console.log("edge strength of probability node: ", edge_strength[effect.id])
                                        let textPath = d3.select("#nrid\\:" + gcause.id + "\\:" + cause.id)
                                        textPath = textPath[0][0]
                                        try {
                                            // handles recent returend values of type double from pywhy causal inf implementation 
                                            textPath.textContent = edge_strength[effect.id][gcause.name].toFixed(4)
                                        } catch (exception) {
                                            // handles stringed format when retrieved from typedb
                                            if (edge_strength[effect.id][gcause.name] !== undefined) {
                                                textPath.textContent = parseFloat(edge_strength[effect.id][gcause.name]).toFixed(4)
                                            }
                                        }
                                    }

RED.sidebar.info.outliner = (function () {
  ...
  function getFlowData() {
    var flowData = [
      ...
      },
      {
        ...
        children: [getEmptyItem("__subflow__")],
      },
      {
        id: "__global__",
        flow: "__global__",
        ...
        types: {},
        children: [getEmptyItem("__global__")],
      },
      {
        id: "__submodel__",
        flow: "__submodel__",
        label: "Causal Submodels",
        types: {},
        children: [getEmptyItem("__submodel__")],
      },
    ];
  ...
  function getProjectLabel(p) {
    ...
    var contentDiv = $("<div>", {
      class: "red-ui-search-result-description red-ui-info-outline-item-label",
    }).appendTo(div);
    contentDiv.text(p.name);
    var controls = $("<div>", {
      class: "red-ui-info-outline-item-controls",
    }).appendTo(div);
    var editProjectButton = $(
      '<button class="red-ui-button red-ui-button-small" style="position:absolute;right:5px;top: 3px;"><i class="fa fa-ellipsis-h"></i></button>'
    )
      .appendTo(controls)
    ...
    RED.popover.tooltip(
      editProjectButton,
      RED._("sidebar.project.showProjectSettings")
    );
    return div;
  }
  var empties = {};
  function getEmptyItem(id) {
    var item = {
      empty: true,
      element: $(
        '<div class="red-ui-info-outline-item red-ui-info-outline-item-empty">'
  ...
  function addControls(n, div) {
    ...
    if (n.type === "subflow") {
      var subflowInstanceBadge = $(
        '<button type="button" class="red-ui-info-outline-item-control-users red-ui-button red-ui-button-small"><i class="fa fa-toggle-right"></i></button>'
      )
        .text(n.instances.length)
        .appendTo(controls)
        .on("click", function (evt) {
          evt.preventDefault();
          evt.stopPropagation();
    ...
    if (n.type === "submodel") {
      var submodelInstanceBadge = $(
        ...
        .on("click", function (evt) {
          evt.preventDefault();
          evt.stopPropagation();
          RED.search.show("type:submodel:" + n.id);
        });
      // RED.popover.tooltip(userCountBadge,function() { return RED._('editor.nodesUse',{count:n.users.length})});
    }
    if (n._def.category === "config" && n.type !== "group") {
      ...
      RED.popover.tooltip(userCountBadge, function () {
        return RED._("editor.nodesUse", { count: n.users.length });
      });
    }

    if (n._def.button) {
      var triggerButton = $(
        '<button type="button" class="red-ui-info-outline-item-control-action red-ui-button red-ui-button-small"><i class="fa fa-toggle-right"></i></button>'
      )
        .appendTo(controls)
        .on("click", function (evt) {
          evt.preventDefault();
    ...
    // })
    if (n.type !== "subflow" && n.type !== "submodel") {
      var toggleButton = $(
        '<button type="button" class="red-ui-info-outline-item-control-disable red-ui-button red-ui-button-small"><i class="fa fa-circle-thin"></i><i class="fa fa-ban"></i></button>'
      )
        .appendTo(controls)
        .on("click", function (evt) {
          evt.preventDefault();
      ...
      RED.popover.tooltip(toggleButton, function () {
        if (n.type === "group") {
          return (
            RED._("common.label.enable") + " / " + RED._("common.label.disable")
          );
        }
        return RED._(
          "common.label." +
          ((n.type === "tab" && n.disabled) || (n.type !== "tab" && n.d)
            ? "enable"
            : "disable")
        );
      });
    } else {
      $('<div class="red-ui-info-outline-item-control-spacer">').appendTo(
        controls
      );
    }
  ...
  function build() {
    var container = $("<div>", { class: "red-ui-info-outline" }).css({
      height: "100%",
    });
    var toolbar = $("<div>", {
      class: "red-ui-sidebar-header red-ui-info-toolbar",
    }).appendTo(container);

    searchInput = $(
      '<input type="text" data-i18n="[placeholder]menu.label.search">'
    )
      .appendTo(toolbar)
      .searchBox({
        style: "compact",
        ...
        options: [
          {
            label: RED._("sidebar.info.search.configNodes"),
            value: "is:config",
          },
          {
            label: RED._("sidebar.info.search.unusedConfigNodes"),
            value: "is:config is:unused",
          },
          {
            label: RED._("sidebar.info.search.invalidNodes"),
            value: "is:invalid",
          },
          {
            label: RED._("sidebar.info.search.uknownNodes"),
            value: "type:unknown",
          },
          {
            label: RED._("sidebar.info.search.unusedSubflows"),
            value: "is:subflow is:unused",
          },
        ],
      });

    projectInfo = $(
      '<div class="red-ui-treeList-label red-ui-info-outline-project"><span class="red-ui-treeList-icon"><i class="fa fa-archive"></i></span></div>'
    )
      .hide()
      .appendTo(container);
    ...
    RED.events.on("submodels:add", onSubmodelAdd);
    RED.events.on("submodels:remove", onObjectRemove);
    RED.events.on("submodels:change", onSubmodelChange);
  ...
  function onFlowAdd(ws) {
    ...
    console.log("WS disabled ", ws.disabled)
    objects[ws.id].element.toggleClass(
      "red-ui-info-outline-item-disabled",
      !!ws.disabled
    );
    objects[ws.id].treeList.container.toggleClass(
      "red-ui-info-outline-item-disabled",
      !!ws.disabled
    );
    updateSearch();
  ...
  function onFlowChange(n) {
    ...
    existingObject.element.toggleClass(
      "red-ui-info-outline-item-disabled",
      !!n.disabled
    );
    existingObject.treeList.container.toggleClass(
      "red-ui-info-outline-item-disabled",
      !!n.disabled
    );
    updateSearch();
  ...
  function onSubmodelAdd(sf) {
    ...
    if (empties["__submodel__"]) {
      empties["__submodel__"].treeList.remove();
      delete empties["__submodel__"];
    }
    submodelList.treeList.addChild(objects[sf.id]);
    updateSearch();
  }
  function onSubmodelChange(sf) {
    var existingObject = objects[sf.id];
    existingObject.treeList.replaceElement(getNodeLabel(sf));
    // existingObject.element.find(".red-ui-info-outline-item-label").text(n.name || n.id);
    RED.nodes.eachNode(function (n) {
      if (n.type == "submodel:" + sf.id) {
        var sfInstance = objects[n.id];
        sfInstance.treeList.replaceElement(getNodeLabel(n));
      }
    });
  ...
  function onNodeChange(n) {
    ...
    var nodeLabelText = RED.utils.getNodeLabel(
      n,
      n.name || n.type + ": " + n.id
    );
    if (nodeLabelText) {
      existingObject.element
        .find(".red-ui-info-outline-item-label")
        .text(nodeLabelText);
    } else {
      existingObject.element
        .find(".red-ui-info-outline-item-label")
    ...
    if (parent !== existingParent) {
      ...
      if (parentItem.children.length === 0) {
        if (parentItem.config) {
          ...
          delete configNodeTypes[
            parentItem.parent.id || parentItem.parent.parent.id
          ].types[n.type];

          if (parentItem.parent.children.length === 0) {
            if (parentItem.parent.id === "__global__") {
              parentItem.parent.treeList.addChild(
                getEmptyItem(parentItem.parent.id)
              );
            } else {
              ...
              if (parentItem.parent.parent.children.length === 0) {
                parentItem.parent.parent.treeList.addChild(
                  getEmptyItem(parentItem.parent.parent.id)
                );
              }
            }
          }
    ...
    if (n._def.category === "config" && n.type !== "group") {
      existingObject.element
        .find(".red-ui-info-outline-item-control-users")
        .text(n.users.length);
    }
  ...
  function onObjectRemove(n) {
    ...
    if (/^subflow:/.test(n.type)) {
      ...
      if (objects[sfType]) {
        objects[sfType].element
          .find(".red-ui-info-outline-item-control-users")
          .text(RED.nodes.subflow(sfType).instances.length);
      }
    }
    if (/^submodel:/.test(n.type)) {
      var sfType = n.type.substring(9);
      if (objects[sfType]) {
        objects[sfType].element
          .find(".red-ui-info-outline-item-control-users")
          .text(RED.nodes.submodel(sfType).instances.length);
      }
    }
        ...
        if (parent.parent.children.length === 0) {
          if (parent.parent.id === "__global__") {
            ...
            if (parent.parent.parent.children.length === 0) {
              parent.parent.parent.treeList.addChild(
                getEmptyItem(parent.parent.parent.id)
              );
            }
          }
        }
  ...
  function getGutter(n) {
    ...
    var revealButton = $(
      '<button type="button" class="red-ui-info-outline-item-control-reveal red-ui-button red-ui-button-small"><i class="fa fa-search"></i></button>'
    )
      .appendTo(span)
      .on("click", function (evt) {
        evt.preventDefault();
  ...
  function createFlowConfigNode(parent, type) {
    ...
    if (!configNodeTypes[parent].types[type]) {
      ...
      configNodeTypes[parent].treeList.addChild(
        configNodeTypes[parent].types[type]
      );
  ...
  function onNodeAdd(n) {
    ...
    objects[n.id].element.toggleClass(
      "red-ui-info-outline-item-disabled",
      !!n.d
    );
    if (/^subflow:/.test(n.type)) {

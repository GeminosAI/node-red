...
RED.sidebar.info = (function () {
    ...
    function init() {
        ...
        propertiesPanelHeaderReveal = $('<button type="button" class="red-ui-button red-ui-button-small"><i class="fa fa-search"></button>').css({
            ...
            console.log("ON CLICK FOR FA FA SEARCH")
            evt.preventDefault();
            evt.stopPropagation();
            if (selectedObject) {
    ...
    function refresh(node) {
        ...
        if (node === null) {
            ...
            if (types.submodels > 0) {
                $('<div>').text(RED._("clipboard.submodels", { count: types.submodels })).appendTo(counts);
            }
        } else {
            ...
            var submodelRegex = /^submodel(:(.+))?$/.exec(node.type);
            if (submodelRegex) {
                if (submodelRegex[2]) {
                    submodelNode = RED.nodes.submodel(submodelRegex[2]);
                } else {
                    submodelNode = node;
                }

                var submodelType = "submodel:" + submodelNode.id;
                submodelUserCount = submodelNode.instances.length;
            }
            ...
            if (node.type === "subflow" || subflowRegex) {
                objectType = "subflow";
            } else if (node.type === "submodel" || submodelRegex) {
                objectType = "submodel"
            ...
            if (node.type === "tab" || node.type === "subflow" || node.type === "submodel") {
                ...
                var count = 0;
                if (!subflowRegex && node.type != "subflow" && node.type != "group" && !submodelRegex && node.type != "submodel") {
                ...
                if (node.type !== 'tab') {
                    ...
                    if (submodelRegex) {
                        $('<tr class="blank"><th colspan="2">' + RED._("sidebar.info.submodel") + '</th></tr>').appendTo(tableBody);
                        $('<tr class="node-info-submodel-row"><td>' + RED._("common.label.name") + '</td><td><span class="red-ui-text-bidi-aware" dir=\"' + RED.text.bidi.resolveBaseTextDir(submodelNode.name) + '">' + RED.utils.sanitize(submodelNode.name) + '</span></td></tr>').appendTo(tableBody);
                    }
            ...
            if (submodelRegex) {
                propRow = $('<tr class="red-ui-help-info-row"><td>' + RED._("submodel.category") + '</td><td></td></tr>').appendTo(tableBody);
                var category = submodelNode.category || "submodels";
                $(propRow.children()[1]).text(RED._("palette.label." + category, { defaultValue: category }))
                $('<tr class="node-info-submodel-row"><td>' + RED._("sidebar.info.instances") + "</td><td>" + submodelUserCount + '</td></tr>').appendTo(tableBody);
                if (submodelNode.meta) {
                    propRow = $('<tr class="red-ui-help-info-row"><td>' + RED._("submodel.module") + '</td><td></td></tr>').appendTo(tableBody);
                    $(propRow.children()[1]).text(submodelNode.meta.module || "")
                    propRow = $('<tr class="red-ui-help-info-row"><td>' + RED._("submodel.version") + '</td><td></td></tr>').appendTo(tableBody);
                    $(propRow.children()[1]).text(submodelNode.meta.version || "")
                }
            }
    ...
    function refreshSelection(selection) {
        ...
        if (selection.nodes) {
            if (selection.nodes.length == 1) {
                ...
                } else if (node.type === "submodel" && node.direction) {
                    refresh(RED.nodes.submodel(node.z))
                } else {
                    refresh(node);
                }
            } else {
        ...
        } else if (selection.flows || selection.subflows || selection.submodels) {
            refresh(selection.flows);
        } else {
            var activeWS = RED.workspaces.active();

            var flow = RED.nodes.workspace(activeWS) || RED.nodes.subflow(activeWS) || RED.nodes.submodel(activeWS);
            if (flow) {
                refresh(flow);
            } else {

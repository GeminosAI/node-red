...
RED.palette = (function () {
  ...
  function createCategory(originalCategory, rootCategory, category, ns) {
    if ($("#red-ui-palette-base-category-" + rootCategory).length === 0) {
      createCategoryContainer(
        originalCategory,
        rootCategory,
        ns + ":palette.label." + rootCategory
      );
    }
    $("#red-ui-palette-container-" + rootCategory).show();
    if ($("#red-ui-palette-" + category).length === 0) {
      $("#red-ui-palette-base-category-" + rootCategory).append(
  ...
  function createCategoryContainer(originalCategory, category, labelId) {
    ...
    var catDiv = $(
      '<div id="red-ui-palette-container-' +
      category +
      '" class="red-ui-palette-category hide">' +
      '<div id="red-ui-palette-header-' +
      category +
      '" class="red-ui-palette-header"><i class="expanded fa fa-angle-down"></i><span>' +
      label +
      "</span></div>" +
      '<div class="red-ui-palette-content" id="red-ui-palette-base-category-' +
      category +
      '">' +
  ...
  function setLabel(type, el, label, info) {
    ...
    for (var i = 0; i < words.length; i++) {
      ...
      var sep = i == 0 ? "" : " ";
      var newWidth = RED.view.calculateTextWidth(
      ...
      if (newWidth < nodeWidth) {
        ...
        while (true) {
          var wordWidth = RED.view.calculateTextWidth(
            word,
            "red-ui-palette-label"
          );
          if (wordWidth >= nodeWidth) {
            ...
            for (var j = word.length; j > 0; j--) {
              ...
              var width = RED.view.calculateTextWidth(
                s,
                "red-ui-palette-label"
              );
              if (width < nodeWidth) {
    ...
    labelElement
      .html(lines)
      .attr("dir", RED.text.bidi.resolveBaseTextDir(lines));

    el.find(".red-ui-palette-port").css({
    ...
    try {
      var l =
        ...
        "</b></p>";
      popOverContent = $("<div></div>").append(
        $(
          l +
          (info ? info : RED.nodes.getNodeHelp(type) || "<p>" + RED._("palette.noInfo") + "</p>"
          ).trim()
        )
          .filter(function (n) {
            return (
              (this.nodeType == 1 && this.nodeName == "P") ||
              (this.nodeType == 3 && this.textContent.trim().length > 0)
            );
          })
          .slice(0, 2)
      );
      ...
      if (typeInfo) {
        var metaData = "";
        if (typeInfo && !/^subflow:/.test(type) && !/^submodel:/.test(type)) {
        ...
        if (/^subflow:/.test(type)) {
          $(
            '<button type="button" onclick="RED.workspaces.show(\'' +
            type.substring(8).replace(/'/g, "\\'") +
            '\'); return false;" class="red-ui-button red-ui-button-small" style="float: right; margin-left: 5px;"><i class="fa fa-pencil"></i></button>'
          ).appendTo(popOverContent);
        }
        if (/^submodel:/.test(type)) {
          $(
            '<button type="button" onclick="RED.workspaces.show(\'' +
            type.substring(9).replace(/'/g, "\\'") +
            '\'); return false;" class="red-ui-button red-ui-button-small" style="float: right; margin-left: 5px;"><i class="fa fa-pencil"></i></button>'
          ).appendTo(popOverContent);
        }
        ...
        $(
          '<button type="button" onclick="RED.search.show(\'type:' +
          safeType +
          '\'); return false;" class="red-ui-button red-ui-button-small" style="float: right; margin-left: 5px;"><i class="fa fa-search"></i></button>'
        ).appendTo(popOverContent);
        $(
          '<button type="button" onclick="RED.sidebar.help.show(\'' +
          safeType +
          '\'); return false;" class="red-ui-button red-ui-button-small" style="float: right; margin-left: 5px;"><i class="fa fa-book"></i></button>'
        ).appendTo(popOverContent);
      ...
      popOverContent =
        "<p><b>" + label + "</b></p><p>" + RED._("palette.noInfo") + "</p>";
    }
  ...
  function addNodeType(nt, def) {
    ...
    if (exclusion.indexOf(nodeCategory) === -1) {
      ...
      var d = $("<div>", { class: "red-ui-palette-node" })
        .attr("data-palette-type", nt)
      ...
      if (typeof def.paletteLabel !== "undefined") {
        try {
          label =
            (typeof def.paletteLabel === "function" ? def.paletteLabel.call(def) : def.paletteLabel) || "";
        } catch (err) {
      ...
      if (def.icon) {
        ...
        var iconContainer = $("<div/>", {
          class:
            ...
            ((!def.align && def.inputs !== 0 && def.outputs === 0) ||
              "right" === def.align ? " red-ui-palette-icon-container-right": ""),
        }).appendTo(d);
        iconContainer.attr("data-palette-icon", icon_url);
        RED.utils.createIconElement(icon_url, iconContainer, true);
      }
      ...
      createCategory(
        ...
        rootCategory,
        category,
        coreCategories.indexOf(rootCategory) !== -1 ? "node-red" : def.set.id
      );
      ...
      $(d).draggable({
        ...
        start: function () {
          ...
          paletteTop =
            $("#red-ui-palette").parent().position().top +
            $("#red-ui-palette-container").position().top;
          hoverGroup = null;
          activeGroup = RED.view.getActiveGroup();
          if (activeGroup) {
            document
              .getElementById("group_select_" + activeGroup.id)
              .classList.add("red-ui-flow-group-active-hovered");
          }
          RED.view.focus();
        },
        stop: function () {
          d3.select(".red-ui-flow-link-splice").classed(
            "red-ui-flow-link-splice",
            false
          );
          if (hoverGroup) {
            document
              .getElementById("group_select_" + hoverGroup.id)
              .classList.remove("red-ui-flow-group-hovered");
          }
          if (activeGroup) {
            document
              .getElementById("group_select_" + activeGroup.id)
              .classList.remove("red-ui-flow-group-active-hovered");
          }
          if (spliceTimer) {
          ...
          if (!groupTimer) {
            groupTimer = setTimeout(function () {
              ...
              if (group !== hoverGroup) {
                ...
                if (group) {
                  document
                    .getElementById("group_select_" + group.id)
                    .classList.add("red-ui-flow-group-hovered");
                }
                hoverGroup = group;
                if (hoverGroup) {
            ...
            if (!spliceTimer) {
              spliceTimer = setTimeout(function () {
                ...
                for (var i = 0; i < nodes.length; i++) {
                  ...
                  if (
                    node.classed("red-ui-flow-link-background") &&
                    !node.classed("red-ui-flow-link-link")
                  ) {
                ...
                if (activeSpliceLink && activeSpliceLink !== bestLink) {
                  d3.select(activeSpliceLink.parentNode).classed(
                    "red-ui-flow-link-splice",
                    false
                  );
                }
                if (bestLink) {
                  d3.select(bestLink.parentNode).classed(
                    "red-ui-flow-link-splice",
                    true
                  );
                } else {
      ...
      if (nt.indexOf("submodel:") === 0) {
        d.on("dblclick", function (e) {
          RED.workspaces.show(nt.substring(9));
          e.preventDefault();
        });
        var submodel = RED.nodes.submodel(nt.substring(9));
        nodeInfo = RED.utils.renderMarkdown(submodel.info || "");
      }
  ...
  function refreshNodeTypes() {
    ...
    RED.nodes.eachSubmodel(refreshSubmodel)
  }
  function refreshSubflow(sf) {
    ...
    paletteLabel.attr(
      "class",
      "red-ui-palette-label" +
      ((!sf._def.align && sf.in.length !== 0 && sf.out.length === 0) ||
        "right" === sf._def.align ? " red-ui-palette-label-right" : "")
    );

    var paletteIconContainer = paletteNode.find(
      ".red-ui-palette-icon-container"
    );
    paletteIconContainer.attr(
      "class",
      "red-ui-palette-icon-container" +
      ((!sf._def.align && sf.in.length !== 0 && sf.out.length === 0) ||
        "right" === sf._def.align ? " red-ui-palette-icon-container-right" : "")
    );
    ...
    if (currentLabel !== sf.name || currentInfo !== sf.info) {
      ...
      setLabel(
        ...
        paletteNode,
        sf.name,
        RED.utils.renderMarkdown(sf.info || "")
      );
    }
  ...
  function refreshSubmodel(sf) {
    var paletteNode = getPaletteNode("submodel:" + sf.id);
    var portInput = paletteNode.find(".red-ui-palette-port-input");
    var portOutput = paletteNode.find(".red-ui-palette-port-output");
    ...
    var currentCategory = paletteNode.data("category");
    var newCategory = sf.category || "submodels";
    if (currentCategory !== newCategory) {
      var category = escapeCategory(newCategory);
      createCategory(newCategory, category, category, "node-red");
  ...
  function init() {
    $(
      '<img src="red/images/spin.svg" class="red-ui-palette-spinner hide"/>'
    ).appendTo("#red-ui-palette");
    $(
      '<div id="red-ui-palette-search" class="red-ui-palette-search hide"><input type="text" data-i18n="[placeholder]palette.filter"></input></div>'
    ).appendTo("#red-ui-palette");
    $(
      '<div id="red-ui-palette-container" class="red-ui-palette-scroll hide"></div>'
    ).appendTo("#red-ui-palette");
    $('<div class="red-ui-component-footer"></div>').appendTo(
      "#red-ui-palette"
    );
    $('<div id="red-ui-palette-shade" class="hide"></div>').appendTo(
      "#red-ui-palette"
    );
    ...
    RED.events.on("registry:node-set-disabled", function (nodeSet) {
      for (var j = 0; j < nodeSet.types.length; j++) {
        ...
        if (
          def &&
          def.onpaletteremove &&
          typeof def.onpaletteremove === "function"
        ) {
    ...
    RED.events.on("submodels:change", refreshSubmodel)
    ...
    sidebarControls = $(
      '<div class="red-ui-sidebar-control-left"><i class="fa fa-chevron-left"></i></div>'
    ).appendTo($("#red-ui-palette"));
    RED.popover.tooltip(
      sidebarControls,
      RED._("keyboard.togglePalette"),
      "core:toggle-palette"
    );
    ...
    userCategories.forEach(function (category) {
      ...
      createCategoryContainer(
        ...
        "palette.label." + escapeCategory(category)
      );
    });
    ...
    var paletteCollapseAll = $(
      '<button type="button" class="red-ui-footer-button"><i class="fa fa-angle-double-up"></i></button>'
    ).appendTo(paletteFooterButtons);
    ...
    RED.popover.tooltip(
      paletteCollapseAll,
      RED._("palette.actions.collapse-all")
    );

    var paletteExpandAll = $(
      '<button type="button" class="red-ui-footer-button"><i class="fa fa-angle-double-down"></i></button>'
    ).appendTo(paletteFooterButtons);
  ...
  function getCategories() {
    var categories = [];
    $("#red-ui-palette-container .red-ui-palette-category").each(function (
      i,
      d
    ) {

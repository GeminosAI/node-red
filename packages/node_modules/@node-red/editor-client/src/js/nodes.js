...
RED.nodes = (function () {
    ...
    for (var n = 0; n < set.length; n++) {
      ...
      if (node.type !== "subflow" && node.type !== "submodel") {
        ...
        if (node.type === "subflow") {
          ...
          var convertedSubmodel = convertSubmodel(node, { credentials: false })
  ...
  newNodes = newNodes.filter(function (n) {
    ...
    if (!options.generateIds) {
      if (!options.importMap[id]) {
        ...
        var existing = allNodes.getNode(id) || configNodes[id] || workspaces[id] || subflows[id] || groups[id] || junctions[id] || submodels[id];
        if (existing) {
  ...
  for (i = 0; i < newNodes.length; i++) {
    ...
    if (def && def.category == "config") {
      ...
      if (createNewIds || options.importMap[n.id] === "copy") {
        ...
        const keepNodesCurrentZ = reimport && n.z && (RED.workspaces.contains(n.z) || RED.nodes.subflow(n.z) || RED.nodes.submodel(n.z))
        if (!keepNodesCurrentZ && n.z && !workspace_map[n.z] && !subflow_map[n.z] && !submodel_map[n.z]) {
          n.z = activeWorkspace;
        }
      }
    ...
    if (n.type !== "workspace" && n.type !== "tab" && n.type !== "subflow" && n.type !== "submodel") {
      ...
      if (!def || def.category != "config") {
        ...
        if (createNewIds || options.importMap[n.id] === "copy") {
          ...
          node.id = n.id;
          const keepNodesCurrentZ = reimport && node.z && (RED.workspaces.contains(node.z) || RED.nodes.subflow(node.z) || RED.nodes.submodel(node.z))
          if (
            !keepNodesCurrentZ && (node.z == null ||
        ...
        if (node.type === "group") {
          ...
          var parentId = n.type.split(":")[1];
          var submodel = submodel_denylist[parentId] || submodel_map[parentId] || getSubmodel(parentId);
          if (!submodel) {
            node._def = {
              color: "#fee",
              defaults: {},
  ...
  for (i = 0; i < new_submodels.length; i++) {
    console.log("new submodels: ", new_submodels)
    n = new_submodels[i];
    n.in.forEach(function (input) {
      input.wires.forEach(function (wire) {
        var link = {

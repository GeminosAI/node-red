...
RED.nodes = (function () {
  ...
  var registry = (function () {
    ...
    var exports = {
      ...
      registerNodeType: function (nt, def) {
        console.log("node type substring: ", nt.substring(0, 8))
        if (nt.substring(0, 8) !== "subflow:" && nt.substring(0, 9) !== "submodel") {
  ...
  var allNodes = (function () {
    ...
    var api = {
      ...
      moveNodesForwards: function (nodes) {
        ...
        var toMove = new Set(nodes.filter(function (n) { return n.type !== "group" && n.type !== "subflow" && n.type !== "submodel" }));
        var moved = new Set();
  ...
  function addNode(n) {
    if (n.type.indexOf("subflow") !== 0 && n.type.indexOf("submodel") !== 0) {
      n["_"] = n._def._;
    } else {
      ...
      if (n.type.indexOf("subflow") === 0) {
        ...
        n["_"] = RED._;
      } else if (n.type.indexOf("submodel") === 0) {
        var submodelId = n.type.substring(9);
        var sf = RED.nodes.submodel(submodelId);
        if (sf) {
          sf.instances.push(sf);
        }
        n["_"] = RED._;
  ...
  function removeNode(id) {
    ...
    if (id in configNodes) {
      ...
      if (node.type.indexOf("submodel:") === 0) {
        var submodelId = node.type.substring(9);
        var sf = RED.nodes.submodel(submodelId);
        if (sf) {
          sf.instances.splice(sf.instances.indexOf(node), 1);
        }
  ...
  function addSubmodel(sf, createNewIds) {
    if (createNewIds) {
      var submodelNames = Object.keys(submodels).map(function (sfid) {
        return submodels[sfid].name;
      });

      submodelNames.sort();
      var copyNumber = 1;
      var submodelName = sf.name;
      submodelNames.forEach(function (name) {
        if (submodelName == name) {
          copyNumber++;
          submodelName = sf.name + " (" + copyNumber + ")";
        }
      });
      sf.name = submodelName;
    }
    submodels[sf.id] = sf;
    allNodes.addTab(sf.id)
    linkTabMap[sf.id] = [];
    RED.nodes.registerType("submodel:" + sf.id, {
      defaults: {
        name: { value: "" },
        env: { value: [] },
      },
      icon: "font-awesome/fa-bolt",
      category: sf.category || "submodels",
      inputs: sf.in.length,
      outputs: sf.out.length,
      color: "#C3E0E5",
      label: function () {
        return this.name || RED.nodes.submodel(sf.id).name;
      },
      labelStyle: function () {
        return this.name ? "red-ui-flow-node-label-italic" : "";
      },
      paletteLabel: function () {
        return RED.nodes.submodel(sf.id).name;
      },
      ...
      oneditprepare: function () {
        if (this.type !== 'submodel') {
          // A submodel instance node
          RED.submodel.buildEditForm("submodel", this);
        } else {
          // A submodel template node
          RED.submodel.buildEditForm("submodel-template", this);
        }
      },
      oneditresize: function (size) {
        if (this.type === 'submodel') {
    ...
    sf.instances = [];
    sf._def = RED.nodes.getType("submodel:" + sf.id);
    RED.events.emit("submodels:add", sf);
  }

  function getSubmodel(id) {
    return submodels[id];
  }

  function removeSubmodel(sf) {
    if (submodels[sf.id]) {
      delete submodels[sf.id];
      allNodes.removeTab(sf.id);
      registry.removeNodeType("submodel:" + sf.id);
      RED.events.emit("submodels:remove", sf);
    }
  }

  function submodelContains(sfid, nodeid) {
    var sfNodes = allNodes.getNodes(sfid);
    for (var i = 0; i < sfNodes.length; i++) {
      var node = sfNodes[i];
      var m = /^submodel:(.+)$/.exec(node.type);
      if (m) {
        if (m[1] === nodeid) {
          return true;
        } else {
          var result = submodelContains(m[1], nodeid);
          if (result) {
            return true;
          }
        }
  ...
  function getAllDownstreamNodes(node) {
    return getAllFlowNodes(node, "down").filter(function (n) {
      return n !== node;
    });
  }

  function getAllUpstreamNodes(node) {
    return getAllFlowNodes(node, "up").filter(function (n) {
      return n !== node;
    });
  }
  ...
  function convertNode(n, opts) {
    ...
    if (n.type === "tab") {
      console.log("converting workspace", convertWorkspace(n))
    ...
    if (node.type == "unknown") {
      ...
      if (exportCreds) {
        var credentialSet = {};
        if ((/^subflow:/.test(node.type) && n.credentials) || (/^submodel:/.test(node.type) && n.credentials)) {
          // A subflow instance node can have arbitrary creds
          for (var sfCred in n.credentials) {
            if (n.credentials.hasOwnProperty(sfCred)) {
              if (
                !n.credentials._ ||
                n.credentials["has_" + sfCred] !=
                n.credentials._["has_" + sfCred] ||
          ...
          for (var cred in n._def.credentials) {
            if (n._def.credentials.hasOwnProperty(cred)) {
              if (n._def.credentials[cred].type == "password") {
                ...
                (!n.credentials._ ||
                  n.credentials[cred] != n.credentials._[cred])
              ) {
                credentialSet[cred] = n.credentials[cred];
              }
            }
    ...
    if (n._def.category != "config") {
      ...
      for (var j = 0; j < wires.length; j++) {
        var w = wires[j];
        if (w.target.type !== "subflow" && w.target.type !== "submodel") {
  ...
  function convertSubmodel(n, exportCreds) {
    var node = {};
    node.id = n.id;
    node.type = n.type;
    node.name = n.name;
    ...
    if (exportCreds) {
      var credentialSet = {};
      // A submodel node can have arbitrary creds
      for (var sfCred in n.credentials) {
        if (n.credentials.hasOwnProperty(sfCred)) {
          if (
            !n.credentials._ ||
    ...
    n.out.forEach(function (p, c) {
      ...
      var wires = links.filter(function (d) { return d.target === p });
      for (i = 0; i < wires.length; i++) {
        if (wires[i].source.type !== "submodel") {
          nOut.wires.push({ id: wires[i].source.id, port: wires[i].sourcePort })
        } else {
          nOut.wires.push({ id: n.id, port: 0 })
        }
    ...
    if (n.icon) {
      if (n.icon !== "node-red/submodel.svg") {
        node.icon = n.icon;
      }
    }
    if (n.status) {
  ...
  function createExportableSubmodel(id) {
    var sf = getSubmodel(id);
    var nodeSet;
    var sfNodes = allNodes.getNodes(sf.id);
    if (sfNodes) {
      nodeSet = sfNodes.slice();
  ...
  function createExportableNodeSet(set, exportedIds, exportedSubflows, exportedConfigNodes) {
    var nns = [];
    console.log("set", set)
    console.log("exportedIds", exportedIds)
    console.log("expoertedSubflows", exportedSubflows)
    console.log("exportedConfigNodes", exportedConfigNodes)
    ...
    for (var n = 0; n < set.length; n++) {
      ...
      if (node.type.substring(0, 9) === "submodel:") {
        var submodelId = node.type.substring(9);
        if (!exportedSubmodels[submodelId]) {
          exportedSubmodels[submodelId] = true;
          var submodel = getSubmodel(submodelId);
          var submodelSet = allNodes.getNodes(submodelId).slice();
          submodelSet.unshift(submodel);
          ...
          var exportableSubmodel = createExportableNodeSet(submodelSet, exportedIds, exportedSubmodels, exportedConfigNodes);
          nns = exportableSubmodel.concat(nns);
        }
      }
      ...
      if (node.type !== "subflow" && node.type !== "submodel") {
        ...
        if (node.type === "subflow") {
          ...
          console.log("convertedSubflow: ", convertedSubflow);
          nns.push(convertedSubflow);
        } else if (node.type === "submodel") {
          var convertedSubmodel = convertSubmodel(node)
          console.log("convertedSubmodel: ", convertedSubmodel);
          nns.push(convertedSubmodel)
        }
      }
  ...
  function createCompleteNodeSet(opts) {
    ...
    for (i in submodels) {
      if (submodels.hasOwnProperty(i)) {
        nns.push(convertSubmodel(submodels[i], opts))
      }
    }
  ...
  function checkForMatchingSubmodel(submodel, submodelNodes) {
    submodelNodes = submodelNodes || [];
    var i;
    var match = null;
    RED.nodes.eachSubmodel(function (sf) {
      if (
        sf.name != submodel.name ||
        sf.info != submodel.info ||
        sf.in.length != submodel.in.length ||
        sf.out.length != submodel.out.length
      ) {
        return;
      }
      var sfNodes = RED.nodes.filterNodes({ z: sf.id });
      if (sfNodes.length != submodelNodes.length) {
        return;
      }

      var submodelNodeSet = [submodel].concat(submodelNodes);
      var sfNodeSet = [sf].concat(sfNodes);

      var exportableSubmodelNodes = JSON.stringify(submodelNodeSet);
      var exportableSFNodes = JSON.stringify(
        createExportableNodeSet(sfNodeSet)
      );
      var nodeMap = {};
      for (i = 0; i < sfNodes.length; i++) {
        exportableSubmodelNodes = exportableSubmodelNodes.replace(
          new RegExp('"' + submodelNodes[i].id + '"', "g"),
          '"' + sfNodes[i].id + '"'
        );
      }
      exportableSubmodelNodes = exportableSubmodelNodes.replace(
        new RegExp('"' + submodel.id + '"', "g"),
        '"' + sf.id + '"'
      );

      if (exportableSubmodelNodes !== exportableSFNodes) {
        return;
      }
  ...
  function identifyImportConflicts(importedNodes) {
    ...
    importedNodes.forEach(function (n) {
      ...
      } else if (n.type === "submodel") {
        imported.submodels[n.id] = n;
  ...
  function replaceNodes(newNodes) {
    ...
    // For each of the submodels to be replaced:
    var newSubmodelIds = Object.keys(newSubmodels);
    newSubmodelIds.forEach(function (id) {
      var n = newSubmodels[id];
      // Get a snapshot of the existing subflow definition
      removedNodes = removedNodes.concat(createExportableSubmodel(id));
      // Remove the old subflow definition - but leave the instances in place
      var removalResult = RED.submodel.removeSubmodel(n.id, true);
      // Create the list of nodes for the new subflow def
      var submodelNodes = [n].concat(zMap[n.id]);
      // Import the new subflow - no clashes should occur as we've removed
      // the old version
      var result = importNodes(submodelNodes);
      newSubmodels[id] = getSubmodel(id);
    });
    ...
    RED.nodes.eachNode(function (n) {
      ...
      } else if (/^submodel:/.test(n.type)) {
        var sfId = n.type.substring(9)
        if (newSubmodels[sfId]) {
          // This is an instance of one of the replaced subflows
          //  - update the new def's instances array to include this one
          newSubmodels[sfId].instances.push(n);
          //  - update the instance's _def to point to the new def
          n._def = RED.nodes.getType(n.type);
          //  - set all the flags so the view refreshes properly
          n.dirty = true;
    ...
    newSubmodelIds.forEach(function (id) {
      var n = newSubmodels[id];
      RED.events.emit("submodels:change", n);
    });
 ...
 */
  function importNodes(newNodesObj, options) {
    ...
    newNodes = newNodes.filter(function (n) {
      ...
      if (!options.generateIds) {
        if (!options.importMap[id]) {
          ...
          var existing = allNodes.getNode(id) || configNodes[id] || workspaces[id] || subflows[id] || submodels[id] || groups[id];
          if (existing) {
    ...
    if (existingNodes.length > 0) {
      var errorMessage = RED._("clipboard.importDuplicate", {
      ...
      if (existingNodesCount !== existingNodes.length) {
        $("<li>")
          .text(
            RED._("deploy.confirm.plusNMore", {
              count: existingNodes.length - existingNodesCount,
            })
          )
          .appendTo(nodeList);
      }
    ...
    for (i = 0; i < newNodes.length; i++) {
      ...
      var c = /^submodel:(.+)$/.exec(newNodes[i].type);
      if (c) {
        var submodelId = c[1];
        var parent = getSubmodel(activeWorkspace);
        if (parent) {
          // console.log("getting submodel parent : ", parent)
          var err;
          if (submodelId === parent.id) {
            err = new Error(
              RED._("notification.errors.cannotAddSubmodelToItself")
            );
          }
          if (submodelContains(submodelId, parent.id)) {
            err = new Error(
              RED._("notification.errors.cannotAddCircularReference")
            );
          }
    ...
    for (i = 0; i < newNodes.length; i++) {
      ...
      if (n.type === "workspace" || n.type === "tab") {
        ...
        var matchingSubmodel;
        if (!options.importMap[n.id]) {
          matchingSubmodel = checkForMatchingSubmodel(n, nodeZmap[n.id]);
        }
        if (matchingSubmodel) {
          submodel_denylist[n.id] = matchingSubmodel;
        } else {
          submodel_map[n.id] = n;
          if (createNewIds || options.importMap[n.id] === "copy") {
            nid = getID();
          ...
          // TODO: handle createNewIds - map old to new submodel ids
          n.in.forEach(function (input, i) {
            input.type = "submodel";
            input.direction = "in";
            input.z = n.id;
          ...
          if (n.status) {
            n.status.type = "submodel";
            n.status.direction = "status";
            n.status.z = n.id;
            n.status.id = getID();
          }
          new_submodels.push(n);
          addSubmodel(n, createNewIds || options.importMap[n.id] === "copy");
        }
      }
    }
      ...
      if (def && def.category == "config") {
        ...
        if (createNewIds || options.importMap[n.id] === "copy") {
          if (n.z) {
            ...
            } else if (submodel_denylist[n.z]) {
              continue;
            } else if (submodel_map[n.z]) {
              n.z = submodel_map[n.z].id;
            } else {
          ...
          if (n.z && !workspace_map[n.z] && !subflow_map[n.z] && !submodel_map[n.z]) {
            n.z = activeWorkspace;
          }
        }
    ...
    for (i = 0; i < newNodes.length; i++) {
      n = newNodes[i];
      // TODO: remove workspace in next release+1
      if (n.type !== "workspace" && n.type !== "tab" && n.type !== "subflow" && n.type !== "submodel") {
        def = registry.getNodeType(n.type);
        if (!def || def.category != "config") {
          var node = {
            x: parseFloat(n.x || 0),
          ...
          if (createNewIds || options.importMap[n.id] === "copy") {
            ...
            if (submodel_denylist[n.z]) {
              continue;
            } else if (submodel_map[node.z]) {
              node.z = submodel_map[node.z].id;
            } else {
            ...
            if (
              node.z == null ||
              (!workspace_map[node.z] && !subflow_map[node.z] && !submodel_map[node.z])
            ) {
              if (createMissingWorkspace) {
                if (missingWorkspace === null) {
                  missingWorkspace = RED.workspaces.add(null, true);
          ...
          if (node.type === "group") {
            ...
            node.env = n.env;
          } else if (node.type.substring(0, 8) === "submodel") {
            var parentId = n.type.split(":")[1];
            var submodel =
              submodel_denylist[parentId] ||
              submodel_map[parentId] ||
              getSubmodel(parentId);
            if (createNewIds || options.importMap[n.id] === "copy") {
              parentId = submodel.id;
              node.type = "submodel:" + parentId;
              node._def = registry.getNodeType(node.type);
              delete node.i;
            }
            node.name = n.name;
            node.outputs = submodel.out.length;
            node.inputs = submodel.in.length;
            node.env = n.env;
          } else {
            if (!node._def) {
              if (node.x && node.y) {
    ...
    // geminos
    for (i = 0; i < new_submodels.length; i++) {
      n = new_submodels[i];
      n.in.forEach(function (input) {
        input.wires.forEach(function (wire) {
          var link = {
      ...
      n.out.forEach(function (output) {
        output.wires.forEach(function (wire) {
          var link;
          if (submodel_map[wire.id] && submodel_map[wire.id].id == n.id) {
            link = {
              source: n.in[wire.port],
              sourcePort: wire.port,
              target: output,
            ...
            link = {
              source: node_map[wire.id] || submodel_map[wire.id],
              sourcePort: wire.port,
              target: output,
            };
          }
  ...
  function clear() {
    ...
    var submodelIds = Object.keys(submodels);
    submodelIds.forEach(function (id) {
      RED.submodel.removeSubmodel(id);
    });
  ...
  return {
    ...
    submodelContains: submodelContains,
    ...
    eachSubmodel: function (cb) {
      for (var id in submodels) {
        if (submodels.hasOwnProperty(id)) {
          if (cb(submodels[id]) === false) {
            break;
          }
        }
      }

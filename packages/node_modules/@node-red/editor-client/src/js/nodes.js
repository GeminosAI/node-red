 ...
 RED.nodes = (function () {
            ...
            for (var d in def.defaults) {
              if (def.defaults.hasOwnProperty(d)) {
                if (def.defaults[d].type) {
                  try {
                    def.defaults[d]._type = parseNodePropertyTypeString(
                      def.defaults[d].type
                    );
                  } catch (err) {
                    console.warn(err);
    ...
    function addNode(n) {
      ...
      if (n._def.category == "config") {
        ...
        if ( (n._def.category == "subflows" || n._def.category == "submodels") && typeof n.i === "undefined" ) {
          var nextId = 0;
        ...
        if (nodeTabMap[n.z]) {
          ...
          console.warn("Node added to unknown tab/subflow/submodel:", n);
        }
      }
    ...
    function removeWorkspace(id) {
      ...
      if (ws) {
        ...
        removedGroups = (groupsByZ[id] || []).filter(function (g) {
          return !g.g;
        });
    ...
    function addSubmodel(sf, createNewIds) {
      ...
      RED.nodes.registerType("submodel:" + sf.id, {
        ...
        oneditprepare: function () {
          ...
          RED.submodel.buildPropertiesForm(this);
        },
        oneditresize: function (size) {
          // var rows = $(".dialog-form>div:not(.node-input-env-container-row)");
          var height = size.height;
          ...
          // var editorRow = $("#dialog-form>div.node-input-env-container-row");
          // height -= (parseInt(editorRow.css("marginTop"))+parseInt(editorRow.css("marginBottom")));
          $("ol.red-ui-editor-submodel-env-list").editableList("height", height);
        },
        set: {
          module: "node-red",
        },
    ...
    function removeSubmodel(sf) {
      if (submodels[sf.id]) {
        console.log("removing submodel in nodes")
     ...
     **/
    function createExportableNodeSet(
      set,
      exportedIds,
      exportedSubflows,
      exportedConfigNodes,
      ...
      exportedSubmodels = exportedSubflows || {};
      for (var n = 0; n < set.length; n++) {
        ...
        if (node.type.substring(0, 9) == "submodel:") {
          ...
          if (!exportedSubmodels[submodelId]) {
            ...
            var exportableSubmodel = createExportableNodeSet(
              submodelSet,
              exportedIds,
              exportedSubmodels,
              exportedConfigNodes
    ...
    function createCompleteNodeSet(exportCredentials) {
      ...
      for (i in submodels){
        if(submodels.hasOwnProperty(i)){
          nns.push(convertSubmodel(submodels[i], exportCredentials))
        }
      }
      return nns;
    }

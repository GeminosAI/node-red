...
module.exports = function (RED) {
    ...
    function FileInNode(n) {
        ...
        function processMsg2(msg, nodeSend, filename, nodeDone) {
            ...
            // console.log(`RED.settings.fileWorkingDirectory ${RED.settings.fileWorkingDirectory}`)
            // console.log(`path is absolue ${!path.isAbsolute(filename)}`)
            // console.log(`process env HOME `, process.env.HOME)
            if (filename && RED.settings.fileWorkingDirectory && !path.isAbsolute(filename)) {
                fullFilename = path.resolve(path.join(RED.settings.fileWorkingDirectory, filename));
                console.log(`Resolved full file name`, fullFilename)
            }
            if (!node.filename) {
                node.status({ fill: "grey", shape: "dot", text: filename });
            }

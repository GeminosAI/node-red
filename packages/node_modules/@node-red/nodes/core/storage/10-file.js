...
module.exports = function (RED) {
    ...
    function FileInNode(n) {
        ...
        this.on("input", function (msg, nodeSend, nodeDone) {
            ...
            var fullFilename = filename;
            var filePath = "";
            console.log("Path is absolute: ", !path.isAbsolute(filename))
            console.log("RED settings file working directory: ", RED.settings.fileWorkingDirectory)
            if (filename && RED.settings.fileWorkingDirectory && !path.isAbsolute(filename)) {
                fullFilename = path.resolve(path.join(RED.settings.fileWorkingDirectory, filename));
            }
            if (!node.filename) {
            ...
            if (filename === "") {
                ...
                var rs = fs.createReadStream(fullFilename)
                    ...
                    .on('error', function (err) {
                        console.log("read stream error caught ")
                        let relativeDirectory = "./"
                        let files = fs.readdirSync(relativeDirectory)

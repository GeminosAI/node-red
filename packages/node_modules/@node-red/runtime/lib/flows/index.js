    ...
    .catch(function (err) {
      if (err.code === "credentials_load_failed" && !storage.projects) {
        ...
        if (err.code === "project_not_found") {
          log.warn(
            log._("storage.localfilesystem.projects.project-not-found", {
              project: err.project,
            })
          );
        } else {
      ...
      if (activeFlowConfig.flows.hasOwnProperty(id)) {
        if (!activeFlowConfig.flows[id].disabled && !activeFlows[id]) {
          ...
          activeFlows[id] = Flow.create(
            flowAPI,
            activeFlowConfig,
            activeFlowConfig.flows[id]
          );
          log.debug("red/nodes/flows.start : starting flow : " + id);
        } else {
          log.debug(
            "red/nodes/flows.start : not starting disabled flow : " + id
          );
        }
      }
    }
...
function stop(type,diff,muteLog,isDeploy) {
  ...
  activeFlowIds.forEach((id) => {
    if (activeFlows.hasOwnProperty(id)) {
      var flowStateChanged =
        diff &&
        (diff.added.indexOf(id) !== -1 || diff.removed.indexOf(id) !== -1);
      log.debug("red/nodes/flows.stop : stopping flow : " + id);
      promises.push(
        activeFlows[id].stop(flowStateChanged ? null : stopList, removedList)
      );
      if (
        type === "full" ||
        flowStateChanged ||
        diff.removed.indexOf(id) !== -1
      ) {
  ...
  for (var id in activeFlowConfig.allNodes) {
    if (activeFlowConfig.allNodes.hasOwnProperty(id)) {
      ...
      if (node.type !== "tab" && node.type !== "subflow" && node.type !== "submodel") {
        ...
        if (
          (subflowDetails && !activeFlowConfig.subflows[subflowDetails[1]]) ||
        ...
        var submodelDetails = submodelInstanceRE.exec(node.type);
        if (
          (submodelDetails &&
            !activeFlowConfig.submodels[submodelDetails[1]]) ||
          (!submodelDetails && !typeRegistry.get(node.type))
        ) {
          if (activeFlowConfig.missingTypes.indexOf(node.type) === -1) {
            activeFlowConfig.missingTypes.push(node.type);
          }
...
function getFlow(id) {
  ...
  // GEM
  if (flow.submodels) {
    var submodelIds = Object.keys(flow.submodels);
    result.submodels = submodelIds.map(function (submodelId) {
      var submodel = clone(flow.submodels[submodelId]);
      var nodeIds = Object.keys(submodel.nodes);
      submodel.nodes = nodeIds.map(function (id) {
        return submodel.nodes[id];
      });
      if (submodel.configs) {
        var configIds = Object.keys(submodel.configs);
        submodel.configs = configIds.map(function (id) {
          return submodel.configs[id];
        });
      }
      delete submodel.instances;
      return submodel;
    });
    if (result.submodels.length === 0) {
      delete result.submodels;
    }
  }
  return result;
}
...
async function updateFlow(id, newFlow, user) {
  console.log(`runtime/lib/flows/index updateFlow id`, id)
  console.log(`runtime/lib/flows/index updateFlow newflow`, newFlow.nodes.length)
  console.log(`runtime/lib/flows/index updateFlow user`, user)
  ...
  if (id === "global") {
    ...
    newConfig = newConfig.filter(function (node) {
      return (
        ...
        (node.hasOwnProperty("z") &&
          activeFlowConfig.flows.hasOwnProperty(node.z))
      );
    });
    ...
    // GEM
    if (newFlow.submodels) {
      // Add in the new submodels
      newFlow.submodels.forEach(function (sf) {
        nodes = nodes.concat(sf.nodes || []).concat(sf.configs || []);
        delete sf.nodes;
        delete sf.configs;
        nodes.push(sf);
  ...
  return setFlows(newConfig, null, "flows", true, null, user).then(function () {
    log.info(
      log._("nodes.flows.updated-flow", {

...
var submodelInstanceRE = /^submodel:(.+)$/;
...
function parseConfig(config) {
   ...
   config.forEach(function(n) {
       ...
       if(n.type === 'submodel'){
           flow.submodels[n.id] = n;
           flow.submodels[n.id].configs = {}
           flow.submodels[n.id].nodes= {}
           flow.submodels[n.id].instances = [];
       }
   });
   var linkWires = {};
   var linkOutNodes = [];
   config.forEach(function(n) {
       if (n.type !== 'subflow' && n.type !== 'tab' && n.type !== 'group' && n.type !=='submodel') {
           var subflowDetails = subflowInstanceRE.exec(n.type);
           var submodelDetails = submodelInstanceRE.exec(n.type);
           ...
           if(subflowDetails){
            ...
            if(submodelDetails){
                if ( (submodelDetails && !flow.submodels[submodelDetails[1]]) || (!submodelDetails && !typeRegistry.get(n.type)) ) {
                    if (flow.missingTypes.indexOf(n.type) === -1) {
                        flow.missingTypes.push(n.type);
                    }
                }
           ...
           } else if (flow.submodels[n.z]){
               container = flow.submodels[n.z];
           }
           if (n.hasOwnProperty('x') && n.hasOwnProperty('y')) {
               ...
               if(submodelDetails){
                   var submodelType = submodelDetails[1]
                   n.submodel = submodelType;
                   flow.submodels[submodelType].instances.push(n)
               }
               if (container) {
   ...
   config.forEach(function(n) {
       if (n.type !== 'subflow' && n.type !== 'tab' && n.type !== 'group' && n.type !== 'submodel') {
           ...
           if (n.z && !flow.subflows[n.z] && !flow.submodels[n.z]) {
    ...
    diffConfigs: function(oldConfig, newConfig) {
                    ...
                    if (removed[node.z] || !newConfig.allNodes.hasOwnProperty(id)) {
                        ...
                        if (!removed[node.z] && newConfig.allNodes[removed[id].z]) {
                            ...
                            if (changed[removed[id].z].type === "subflow") {
                                ...
                                //delete removed[id];
                            }else if(changed[removed[id].z].type === "submodel"){
                                changedSubmodels[removed[id].z] = changed[removed[id].z];
                            }
                        }
                    } else {
                        if (added[node.z]) {
                            ...
                            if (diffNodes(node,newConfig.allNodes[id]) || newConfig.allNodes[id].credentials) {
                                ...
                                }else if (changed[id].type === "submodel"){
                                    changedSubmodels[id] = changed[id];
                                }
                                // Mark the container as changed
                                if (newConfig.allNodes[changed[id].z]) {
                                    ...
                                    if (changed[changed[id].z].type === "subflow") {
                                        ...
                                        delete changed[id];
                                    }else if (changed[changed[id].z].type === "submodel") {
                                        changedSubmodels[changed[id].z] = changed[changed[id].z];
                                        delete changed[id];
                                    }
                                }
                            ...
                            if (!redUtil.compareObjects(node.wires,newConfig.allNodes[id].wires)) {
                                ...
                                if (newConfig.allNodes[wiringChanged[id].z]) {
                                    ...
                                    }else if (changed[wiringChanged[id].z].type === "submodel") {
                                        changedSubmodels[wiringChanged[id].z] = changed[wiringChanged[id].z];
                                        delete wiringChanged[id];
                                    }
                                }
                            }
                ...
                if (!oldConfig.allNodes.hasOwnProperty(id)) {
                    ...
                    if (newConfig.allNodes[added[id].z]) {
                        ...
                        if (changed[added[id].z].type === "subflow") {
                            ...
                            delete added[id];
                        }else if (changed[added[id].z].type === "submodel") {
                            changedSubmodels[added[id].z] = changed[added[id].z];
                            delete added[id];
                        }
                    }
                }
                            ...
                            if (changeOrigin || removed[node[prop]]) {
                                if (!changed[node.id]) {
                                    ...
                                    if (newConfig.allNodes[node.z]) {
                                        ...
                                        if (changed[node.z].type === "submodel") {
                                            changedSubmodels[node.z] = changed[node.z];
                                        }
                                    }
                                }
                            }
        ...
        for (id in newConfig.allNodes) {
            if (newConfig.allNodes.hasOwnProperty(id)) {
                ...
                }else if (newConfig.allNodes[node.z] && newConfig.allNodes[node.z].type === "submodel") {
                    delete changed[node.id];
                }
            }
        }
        ...
        var changedSubmodelStack = Object.keys(changedSubmodels);
        while (changedSubmodelStack.length > 0) {
            var submodelId = changedSubmodelStack.pop();
            for (id in newConfig.allNodes) {
                if (newConfig.allNodes.hasOwnProperty(id)) {
                    node = newConfig.allNodes[id];
                    if (node.type === 'submodel:'+submodelId) {
                        if (!changed[node.id]) {
                            changed[node.id] = node;
                            if (!changed[changed[node.id].z] && newConfig.allNodes[changed[node.id].z]) {
                                changed[changed[node.id].z] = newConfig.allNodes[changed[node.id].z];
                                if (newConfig.allNodes[changed[node.id].z].type === "submodel") {
                                    // This subflow instance is inside a subflow. Add the
                                    // containing subflow to the stack to mark
                                    changedSubflowStack.push(changed[node.id].z);
                                    delete changed[node.id];

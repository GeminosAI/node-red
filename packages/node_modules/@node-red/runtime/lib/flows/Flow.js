 ...
 **/
 ...
 class Flow {
       ...
       if (this.flow.nodes.hasOwnProperty(id)) {
         ...
         if (node.d !== true) {
           if (!node.subflow && !node.submodel) {
             ...
             } else if (node.submodel) {
               if (!this.submodelInstanceNodes[id]) {
                 try {
                   console.log(this.flow.submodels);
                   var submodelDefinition =
                     this.flow.submodels[node.submodel] ||
                     this.global.submodels[node.submodel];
                   // console.log("NEED TO CREATE A SUBFLOW",id,node.subflow);
                   this.submodelInstanceNodes[id] = true;
                   ...
                   this.submodelInstanceNodes[id] = submodel;
                   submodel.start();
                   this.activeNodes[id] = submodel.node;
                 } catch (err) {
   ...
   stop(stopList, removedList) {
     ...
     for (i = 0; i < stopList.length; i++) {
       ...
       if (node) {
         ...
         if (this.submodelInstanceNodes[stopList[i]]) {
           delete this.submodelInstanceNodes[stopList[i]];
         }
         try {
   ...
   getNode(id, cancelBubble) {
     ...
     if (
     ...
     } else if (this.submodelInstanceNodes[id]) {
       return this.submodelInstanceNodes[id];
       ...
       for (var sfId in this.submodelInstanceNodes) {
         if (this.submodelInstanceNodes.hasOwnProperty(sfId)) {
           node = this.submodelInstanceNodes[sfId].getNode(id, cancelBubble);
           if (node) {
             return node;
           }
         }
   ...
   handleStatus(node, statusMessage, reportingNode, muteStatusEvent) {
     ...
     if (!muteStatusEvent) {
       if (
         statusMessage.hasOwnProperty("text") &&
